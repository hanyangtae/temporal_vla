...
- Detector–steering 공동 작업에서는 `docs/collaboration/WORKING_AGREEMENT.md`의 최초 협업 전달 조건을 먼저 읽는다. 데이터·입력·평가 조건의 확정/미결 상태와 작업 소유 범위를 구분한다.
- GR00T serve/eval 발사·예약 변경 전 `docs/05_gpu_server_rules.md`를 반드시 확인한다. A100 GPU당 6모델·모든 세션 합산 서버별 GPU 최대 1장, kanu GPU당 2모델·모든 세션 합산 최대 3장; 다른 모델에 자동 적용하지 않는다.
- GR00T 수집/eval은 공통 하네스를 통과해야 한다. `configs/harness/gpu_policy.json`이 기계 검증 기준이며 사용법은 `docs/collab_codex/runbook_groot_harness.md`를 따른다. 원격 세션별 별도 GPU 원장을 만들지 않는다.
