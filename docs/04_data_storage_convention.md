...
## 3.1 세팅 = 좌표 (`<plan_id>/<machine>/<instruction>/s<sid>/j<jid>/n<nid>`)
...
### 3.1.1 scene·jitter 층위 (2026-09-03 v6 개정; 구 k-층·평탄 si 는 legacy)
...
- **좌표 = 3층 폴더** `<plan_id>/<machine>/<key>/s<sid>/j<jid>/n<nid>/<arm>/`. jid 는 j 인덱스
  (reset_idx 가 아님). 셀 키 `key|s<sid>|j<jid>|n<nid>`. 경로·키·해석은 `src/collect/plan.py` 단일 출처.
- **env 주방 목록**은 plan `extra.env_kwargs.layout_and_style_ids` 에 기록한다(v6 = target 10주방).
  목록이 다르면 같은 seed 라도 다른 세계다 — legacy 5주방 plan 과 섞지 않는다.
- **meta.json / 인덱스**: `scene_idx`·`jitter_idx`·`noise_idx` + 출처(`env_seed`, `jitter_reset_idx`,
  `base_lat`, `base_back`, `layout_id`, `style_id`, `side`, `lang`). legacy k-층(`s<i>/k<r>/n<j>`)과
  2축은 인덱서가 읽기만 한다(`legacy=1`).
- `ep_meta/<task>/<env>--seed<es>.json` 은 base seed 키로 동봉(기록·검증용).
