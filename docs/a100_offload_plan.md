# A100(srv50)(구 worker2) 오프로드 플랜 — apple phase 분산 + 데이터 환류

> 2026-07-07 작성. 전제: A100 스모크(serve+수집 1판) 통과. 대상: **지금 돌고 있는
> scene-seed 매트릭스의 apple phase(A-S3)** 일부를 srv50로 분산.
> 데이터 흐름 = srv50 → (arm 단위 즉시) → 로컬(집계·정리) → 승준 서버(아카이브).
...
- 수집(A-S1)·fit(A-S2)은 **전부 로컬 유지** (xa/gx pool fit이 bread+apple 원본을 한곳에 요구).
  srv50에는 **fit 산출 NPZ만 push** (수백 MB).
- srv50 실행 = `heldout_round_cell.sh` 재사용 + **host-serve 래퍼 신규 파일** (실행 중 스크립트 무수정).
- 환류 = 로컬 pull-watcher가 **arm 완료 단위로 rsync pull → 검증 → srv50 원본 삭제** (디스크 49GB 제약).
...
### srv50 (GPU 2·3만, 0·1 불가침)
- **신규** `scripts/safe/groot_n15/robocasa/steer/master_apple_srv50.sh`:
  - cell 2행(s100084, s100104), `APPLE_SETS` 9개 armset 동일 정의.
  - **GPU 정책(2026-07-07 사용자 지시)**: serve는 **GPU 한 개에만 몰아 싣기** (GPU2 단독, GPU3 비움).
    개수 한도 = VRAM이 아니라 **srv50 총 CPU 60% 이하** (serve+collector 쌍당 ~2~2.5코어;
...
- 수정 내용: A-S3 lane 재배치 —
  ...
  - s100084/s100104 행 제거(srv50 담당).
- 재발사 시 `.S1B_DONE` 등 stage sentinel로 bread 구간 자동 스킵, 완료 arm은 체인 dedup으로 fast-skip.

## 3. 데이터 환류 (3-hop)

```
srv50(생산) --arm 완료마다--> 로컬(정본·집계) --cell 완료마다--> 승준(아카이브)
```

1. **srv50→로컬** — **신규** `scripts/safe/groot_n15/robocasa/steer/pull_srv50_arms.sh` (로컬, setsid detached):
   - 10분 폴링: srv50 `final_chain.log`의 `DONE ->` arm 검출 → 해당 arm 디렉토리 rsync pull
     (표준 경로 `steer_eval/<cell>/<arm>/` 동일 유지) → **검증(rc=0 + pkl 60개 + tsv 행수)** →
     srv50 쪽 pkl/mp4 삭제(tsv·log 유지). srv50 상주 사용량 ≤ ~2GB/lane (49GB 제약 대응).
   - 미검증 삭제 금지. 실패 시 재시도 큐, 3회 실패 시 보고만.
2. **로컬 처리**: 정본이 로컬에 모이므로 기존 흐름 그대로 — per-arm `sr_result_heldout.tsv` 누적,
   전 arm 완료 시 `aggregate_final_scene.py`(8-cell 최종 + Notion) 실행. **srv50산 cell은 집계
...
| 시점 | 액션 |
|---|---|
| 지금~bread 완료(~12h) | 스모크 마무리 → srv50 래퍼·pull-watcher 작성 → (승인 시) 기존 bread NPZ로 srv50 1-arm 드라이런 |
| FINAL2_BREAD_DONE | master kill → lane 재배치 수정 → 재발사 (apple A-S1·A-S2 로컬 진행 ~4h) |
| A-S2 완료 | NPZ push → srv50 `master_apple_srv50.sh` 발사, 로컬 pull-watcher 발사 |
| 이후 | srv50 ~17h / 로컬 ~23h+gx — arm 단위 환류 상시 |
| 전 arm 완료 | 최종 집계+Notion → cell 단위 승준 아카이브 → 로컬 정리 |

기대 효과: apple phase 벽시계 **~46h → ~23h** (srv50 17h + 로컬 재배치 23h, 병렬).

## 5. 함정·안전

- 실행 중 로컬 스크립트 수정 금지 → **모든 어댑터는 신규 파일**, master 수정은 kill 후에만.
- srv50 디스크 99% — pull-후-삭제 엄수, 발사 전 `df` 확인, watcher 죽으면 lane도 세울 것(디스크 폭주 방지 위해 srv50 스크립트에 여유 <10GB 시 pause 가드).
- stale sentinel(`FINAL2_*_DONE`) 재발사 전 정리; waiter는 세션 소멸 주의(PID 기반 재설치).
- serve 종료 시 GPU 반납 확인(cleanup-policy), srv50 GPU0·1 절대 미사용.
...
1. **calibration = bread를 A100에서 재측정** (로컬 추가 실행 불필요 — bread 로컬 완주 결과가 이미 존재):
   `ppcc_bread`(s100084)의 `ho_base` + 대표 arm(`ho_permps60`, `ho_gatedps15`)을 srv50에서 동일
