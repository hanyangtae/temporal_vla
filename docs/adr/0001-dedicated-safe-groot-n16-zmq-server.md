...
For SAFE wiring with the GR00T N1.6 base checkpoint, we will add a dedicated ZMQ feature server under `scripts/safe/groot_n16/robocasa/serve/` instead of extending the general FastAPI GR00T server. This keeps SAFE instrumentation separate from the normal serving API and avoids mixing policy feature export with the user-facing `/act` contract. This is N1.6-specific infrastructure; `scripts/safe/groot_n15/robocasa/` is an eval/split/helper tree, not a local backend library or SAFE feature-server counterpart.
