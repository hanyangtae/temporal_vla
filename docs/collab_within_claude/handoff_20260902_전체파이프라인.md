...
## 7. 새 세션 프롬프트
"main checkout ~/pkt_ws/temporal_vla (branch feat/rs-steer-v4)에서 중추(전체 파이프) 세션 이어받아줘. 정본 = docs/collab_within_claude/handoff_20260902_전체파이프라인.md. 게이팅 설계 = docs/steering/47. §1 판정 재론 x. §6-1(데이터 재수집)부터. 협업 세션은 ListAgents."

## 6c. 09-04~07 v6 LOKO 구제 라운드 (중추 세션 컨텍스트 소진 직전 기록)

- 설계(사용자 확정): 단위 (instruction, scene, 대상 j), pool = 타 j 전판 + 대상 j 실패(성공만 제외), 셀 규칙 pool 실패≥3(+성공≥9·혼합 지터≥2는 제안, 미확정), arm = jfair β{0.6..1.0} + plain β0.8 + reseed(rsn_llr 제외), 서버당 GPU 1장(srv 6 serve·kanu 3장×2).
- 자산: 셀표 `configs/collect/n15_grid_v6_scene_jitter/v6_loko_cells{,_eval,_oldkeys}.tsv`(생성기 `scripts/steer/online_gated/select_loko_cells.py`), 오케스트레이터 `scripts/steer/online_gated/og_v6_expand.sh`(hostname 판별·자기 lease·슬롯 할당기·포트 회피·ART_SLUG_MAP·RUN_BASE), 집계 `agg_v6_expand.py` → `outputs/analysis/v6_expand_agg/`, detector 53 ckpt `outputs/analysis/grid_phase/detector_v6/loko/<stem>/…` + `cells_summary.tsv`, setM `outputs/steer/online_pipe_v4_pilot/instr_setm_v6_gt{,_plain}/`(신 키), 정식 AE 승준 `analysis/grid_phase_v6/ae_k8/`(11키, exclude 5 cluster).
- 키 교환: 아카이브 rebase 08f1c9df8207(oven·washer left↔right). **eval 은 구 plan 77e745c37b0f·구 키로 돌렸다** — 집계는 매핑 적용(신 키 표기). 로컬 메인 트리 plan/collector 구판 유지, origin/dev 새 side 부호 머지 보류.
- 결과(kanu·srv48 23셀 114판): jfair β0.9 38/114(0.33)·plain β0.8 37(0.32)·reseed 23(0.20). 초기조건형 60판 setM 0.43~0.57 vs reseed 0.30, 실행표류형 54판 전 arm ≈ reseed(0.06~0.09). drawer-L 최고, drawer-R 0/30, oven-R reseed 우세. 파손 축 미측정(대상 j 성공 116판에 arm 미투입).
- srv50(PPCC 26셀 111판, GPU1 6 serve) 09-07 새벽 실행 중 — 판당 ~8분(timeout)이라 ~17h; arm 3개로 줄일지 사용자 판단 대기. 결과 회수 = `rsync … AISem_50_junhyeong:~/pkt_ws/temporal_vla/outputs/eval/robocasa/groot_n15/og_v6_expand/ outputs/tmp/v6_results/srv50/og_v6_expand/` 후 agg 재실행(--results-root 3개, --cells $T/cells_eval_all.tsv + PPCC 행).
- 뷰어: https://claude.ai/code/artifact/c86296ad-acef-4f77-930d-bf7abbd38f8a (생성기 job tmp `build_viewer.py`, 영상 `outputs/tmp/v6_viewer/vid/`). PPCC 완료 후 재생성.
- 함정(신규): pkill 자기매칭은 경로 문자열도 걸림(패턴 분할), bash 동적 스코프(루프 변수 local), 카운터식 GPU 배정 OOM, 타 프로세스 포트 8898/8899, ART_SLUG_MAP 다중 매핑은 첫 항목만 동작.
- 남은 것: srv50 완주·집계·뷰어 갱신, 파손 pass(사용자 결정), ck8 arm pass(연산자 ck8 fit 후·rack 각주·marsh 제외), 로컬 구 키 NPZ/detector `_prekeyswap` 정리 + 승준 `_prerebase` 삭제, docs/steering/48, dev PR(eval_whole_pipe_1 → dev).
