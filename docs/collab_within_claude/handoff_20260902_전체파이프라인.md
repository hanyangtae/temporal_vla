...
### 진행 중·남은 것 (09-07 새벽)
- srv50 PPCC 26셀 111판 × 7 arm 실행 중(GPU1, ~17h; arm 3개로 축소 여부 사용자 판단). 완료 → rsync → agg 3 root → 뷰어 재생성.
- 미결(사용자): 셀 규칙(pool 성공≥9·혼합 지터≥2 추가), PPCC arm 축소, 파손 pass(대상 j 성공 116판 × arm), ck8 arm pass(연산자 ck8 fit 후; rack 계열 scene 조건화 각주·marshmallow 제외), GPU 예외.
- 정리: 로컬 구 키 NPZ(`instr_setm_v6_gt*/OvenRack_out-left,DishwasherRack_out-right`)·detector `_prekeyswap/`·승준 `_prerebase/` 삭제(eval 종료 후), 오케스트레이터 tmp/repo 동기화, docs/steering/48 라운드 문서, eval_whole_pipe_1 → dev PR.

## 7. 새 세션 프롬프트
"중추(전체 파이프) 세션 이어받아줘. 메인 트리 ~/pkt_ws/temporal_vla 는 dev, 통합 브랜치는 eval_whole_pipe_1(origin 에도 push 됨; 타 세션 머지는 임시 worktree). 정본 = docs/collab_within_claude/handoff_20260902_전체파이프라인.md §6c(운영 인계)·§0(역할)·§3~4(규약·함정) + docs/05_gpu_server_rules.md. 협업 세션은 ListAgents 의 `데이터 추가 수집`·`action phase`·`연산자 설계`·`fail detector`. 지금 srv50 에서 PPCC eval 이 돌고 있으니 먼저 `ssh AISem_50_junhyeong` 으로 run.log(rc=·DONE) 와 lease 를 확인하고, 끝나면 rsync→agg_v6_expand.py 3 root 집계→뷰어 갱신. 결정이 필요한 건 사용자에게 묻고, 발사·정지는 §6c 절차(pkill 분할 패턴) 그대로."
