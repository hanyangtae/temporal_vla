...
| 실행 식별 | scene/j/n·seed·성공 여부만으로 동일 실행/영상으로 조인하지 않는다. 아래 실행 식별 조건을 모두 기록·대조한다. 동일 영상 파일은 전체 SHA256으로 확인한다. 재실행은 run ID를 구분하되 실제 내용의 동일성은 별도로 검증한다. |
...
재인코딩·편집본은 원본 hash와 변환 대응을 가진 파생 영상이다. 재실행은 별도 run ID를 부여하되,
동일 조건의 결정적 실행은 동일 결과를 기대한다. 궤적·영상 일치를 검증해 원본과 연결하며,
같은 내용으로 확인된 재현본을 독립 표본 두 개로 세지 않는다. 같은 seed만으로 일치를 가정하지 않는다.
