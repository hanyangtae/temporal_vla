...
같은 instruction/s/j/n이라도 재실행에는 별도 run ID를 부여한다. 동일 조건의 결정적 실행은
동일 결과를 기대하며, 실제 궤적·영상의 일치를 검증해 원본과 연결한다. 검증 전에는 좌표나 seed만으로
동일 데이터로 간주하지 않고, 동일 내용으로 확인된 재현본은 독립 표본으로 중복 집계하지 않는다.
`scene_idx`만으로 주방을 조인하지 않고 원본 plan 및 layout/style을 함께 보존한다.
원본과 재수집 영상의 라벨을 좌표만으로 섞지 않는다.

## 식별값별 용도

| 값 | 식별·확인하는 대상 | 사용 시 주의사항 |
|---|---|---|
| `pkl_sha256` | PKL 파일 전체 내용. 동일 파일 확인, 전송 손상·교체 검출, manifest에서 파일 조회·연결 | 영상 hash가 아니다. 저장 시각·metadata·직렬화 차이로 달라질 수 있으므로 hash 불일치만으로 궤적이 다르다고 단정하지 않는다. |
| `sig` | 기존 파일의 짧은 조회·대조용 값 | 현재 temporal_vla `src/collect/artifacts.py`에서는 PKL SHA256 앞 16자리다. 단독 전역 고유 ID로 쓰지 말고 원본 좌표·전체 hash와 함께 확인한다. 다른 포맷은 생성 규칙을 먼저 확인한다. |
| `video_sha256` | 영상 파일 전체 내용 | 같은 영상 파일인지 확인한다. 재인코딩하면 hash가 바뀔 수 있으므로 원본/파생 관계를 별도로 기록한다. |
| `run_id` | 데이터를 생성한 실행 1회 | 재실행마다 구분한다. 다른 run ID라도 결정적 재현으로 내용이 같을 수 있으므로 ID 차이가 궤적 차이를 의미하지 않는다. |

PKL·영상·activation·라벨은 manifest에서 서로 연결한다. 따라서 PKL hash로 대응 영상을
찾을 수는 있지만, 그 hash 자체가 영상의 동일성을 증명하지는 않는다.
`video_sha256`·`run_id`는 공유 시 필요한 논리 필드이며 모든 기존 meta에 이미 존재한다는 뜻은 아니다.
...
- **개발에 필요한 데이터가 부족하거나 추가 activation이 필요하면 박경태에게 요청한다.**
- 필요한 task/instruction·scene, 성공/실패 구성, 희망 수량, layer·denoise·token 등 activation 범위와 사용 목적을 함께 전달한다.
- 구체적인 범위가 아직 불명확하면 부족한 점과 목적을 먼저 전달해 수집·제공 범위를 조율한다.
