...
| SAFE feature export | HTTP path는 `/act` only, SAFE feature는 ZMQ 전용 | HTTP serve가 `/act_with_features`도 노출 (`features.*` namespace, `features.hidden_states` feature blob). 같은 모델 인스턴스 공유. ZMQ `get_action_with_features`는 upstream SAFE collector msgpack 호환을 위해 유지 |
| SAFE feature 공유 모듈 | DiT hook이 ZMQ feature_server.py에 inline | `src/policies/groot/safe/features.py` 가 HTTP/ZMQ 양쪽이 공유하는 hook + horizon 해석을 맡고, feature blob encode/decode 규약은 `src/utils/common/feature_blob.py` 로 분리 |
...
일반 계약 (요청 payload, `features.*` namespace, feature blob 형식) 은 [`../01_serving_interface.md`](../01_serving_interface.md#act_with_features) 참조. 아래는 GR00T N1.6 한정 사실:
...
- RoboCasa PandaOmron 기준 `model_action_horizon = 50`, `valid_action_horizon = 16`, `exported_action_token_count = feature_action_horizon`, K denoising step 은 N1.6 sampler 설정에 따름.
...
- DiT 캡처 로직은 `src/policies/groot/safe/features.py` 에 단일화돼 ZMQ feature_server 와 동일한 텐서 정의를 보장한다 (HTTP/ZMQ 둘 다 같은 hidden state 정의).

### SAFE feature 공유 모듈

`src/policies/groot/safe/features.py` 가 GR00T N1.6 SAFE feature capture의 단일 진입점이다.
...
- `encode_feature_tensor_blob(tensor, dtype)` / `decode_feature_tensor_blob(blob)` — GR00T tensor dtype cast 후 공통 `src/utils/common/feature_blob.py` 규약으로 직렬화/복원.

정책/version을 가로지르는 `feature_kind`/`feature_axes`/horizon alias normalization은
`src/policies/safe_metadata.py` 에 둔다.
...
| SAFE feature 공유 모듈 | `src/policies/groot/safe/features.py`, `src/policies/safe_metadata.py`, `src/utils/common/feature_blob.py`, `scripts/safe/groot_n16/robocasa/serve/feature_server.py` |
...
| Tests | `tests/test_groot_safe_features.py`, `tests/test_feature_blob.py`, `tests/test_safe_metadata.py`, `tests/test_serve_groot.py` (/act_with_features), `tests/test_vla_client.py` (predict_with_features) |
