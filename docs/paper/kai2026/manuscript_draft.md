...
(제7회 한국 인공지능 학술대회 투고 초안 — 1~2쪽. 저자 목록·소속은 사용자 확정 사항.
수치는 전부 `numbers.md` 대조표 경유. [S①][S②] 표시는 보강 실험 결과 대기 자리.)
...
수집 데이터에서도 재현된다(z +4.3~+5.1). 즉 VLA 활성화에는 사람의 단계 정의보다 세밀
하지만 단계 정합적인 하위 구조가 실재하며, 스텝 단위 온라인 판독이 가능하다.

## 1. 서론

로봇 조작 태스크는 통상 reach → grasp → transport 같은 primitive 단계(phase)로
기술된다. VLA 모델의 내부 활성화가 이런 단계 구조를 담고 있는지, 담고 있다면 어떤
해상도로 담는지는 열려 있는 질문이다. 내부 단계를 읽을 수 있으면 단계 조건부 개입,
실패 감지 등 추론 시 제어의 기초가 된다. 본 연구는 라벨 없는 군집화만으로 활성화의
시간 구조를 조사하고, 사람이 붙인 단계 라벨(GT)과의 관계를 정량화한다.

## 2. 방법

**데이터·특징.** RoboCasa 주방 조작 9개 태스크에서 GR00T N1.5 rollout을 수집하고,
action head(DiT) 잔차 스트림 layer 12, denoising step 3의 49개 토큰 활성화를 평균해
스텝당 1536차원 특징을 얻는다. 사람 주석 GT 단계 라벨(태스크당 5~8단계)을 평가에만
...
**활성화 상태는 GT 단계보다 3~4배 세밀하다.** 발견 상태의 평균 구간 길이는 4.3 스텝
으로 GT 단계(16.1 스텝)의 약 1/4이다(전환율 18.4% vs 5.0%; 데이터셋 B에서도 6.0 vs
18.1 스텝, 전환 수 3.5배). 이 전환은 고정 리듬 아티팩트가 아니고(전환 위치의 주기성
없음), 전환 시점이 GT 경계와 정렬되지도 않는다(z −1.0~+0.5 — 우연 수준). 그러나
발견 상태를 GT 단계 단위로 병합하면 경계 정렬이 유의해진다(z +1.3~+4.9). 즉 활성화는
GT 단계를 그대로 재현하는 것이 아니라 그보다 잘게 쪼갠다.

**각 군집은 특정 단계에 국한된다.** 한 단계에서 나타난 군집은 다른 단계에서 잘
나타나지 않는다: 군집→단계 순도 0.80, MI 2.17 bits(상한 2.82), 시간 대조군 margin
+1.665 bits. 여러 군집이 한 단계를 나눠 가지므로 대응은 1:1이 아니라 다대일이다 —
이것이 위의 "세밀한 하위 분할"과 순도가 공존하는 이유다. [S② contingency 정량:
군집별 최빈 단계 점유율 분포·단계당 군집 수 — 보강 실행 후 수치 기입]
활성화에는 장면(scene) 정보도 섞여 있으나(태스크별 mi_scene 0.2~1.0 bits), [S①
장면별 평균을 제거한 잔차에서도 단계 margin이 유지됨 — 보강 실행 후 수치 기입]
단계 정보는 장면 암기로 환원되지 않는다.
...
구조는 단계 조건부 개입과 실패 감지의 기초 신호로 쓸 수 있다.
