...
활성화는 GT 단계를 그대로 재현하는 것이 아니라 그보다 잘게 쪼갠다. 이 세분화는 군집
수 k를 크게 잡은 산술적 결과가 아니다: GT 단계 수(3~6)에 가까운 k=8에서도 구간
길이는 GT보다 짧게 유지되고(10개 중 9개 태스크), 에피소드당 전환 수가 상태를 한 번씩
지나는 단조 통과의 상한(k−1=7회)을 넘는 태스크가 절반이다(최대 22회) — 상태가
재방문되는 구조로, k만으로는 만들 수 없다. 반대로 한 태스크(OvenRack)는 같은 k=8에서
GT보다 굵게 병합되어, 해상도는 k가 아니라 활성화가 정한다.
...
— 이것이 위의 "세밀한 하위 분할"과 순도가 공존하는 이유다. 비디오 행동 분절에서
과분할은 통상 벌점이지만, 여기서 세분은 phase-순수한 다대일 대응이며 그 판정 근거는
순도·MI·margin이지 경계 F1이 아니다.
