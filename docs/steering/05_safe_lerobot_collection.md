...
> **⚠ 스코프 주의 (2026-07-30)**: 이 문서는 **steering 전용이 아니라 수집 인프라 문서**다.
> 여기 적힌 모델×벤치 조합(예: pi0.5×libero)은 "그 조합이 이렇다"는 **사양 기록**이지
> "그걸 한다"는 계획이 아니다. 실행 계획으로 읽지 말 것.
>
> **최신화 필요**: 데이터 한 점(datapoint)의 의미 정의와 운영 제약 절이 현행 수집 방식과
> 어긋난다. 갱신 전까지는 경로·도구 정보만 신뢰할 것.
