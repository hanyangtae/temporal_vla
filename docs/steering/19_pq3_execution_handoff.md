...
**`/home/dongkyu/.claude/plans/dynamic-riding-aurora.md` (v9 승인본)** — pq3 라운드의 설계 전체가 여기 있다.
...
- **pq2 P3 라운드 완주·종결**: 115 arm × 60판 전부 완료(ledger), 결론 = 모든 steering arm이 위약 동률(null).
  집계: `outputs/eval/robocasa/groot_n15/steer_eval_pq2/aggregate_v2/` (arms=115).
- **eval activation 전면 삭제됨** (3 호스트, 22,675개/~172GB): steer_eval*·pq2 p2/p3의 rollout pkl/zst는
  로컬·w2(.50)·승준(.37) 어디에도 없다. 판정은 dual_scoring.tsv 사이드카(115/115) + 파일명 succ 스템(.csv 마커)로 유지
...
| cell 테이블 | `scripts/safe/groot_n15/robocasa/steer/pq/pq_lib.sh` CELLS (pq3_ 5행 추가 필요) |
...
| pq2 파생 원본들 | `scripts/safe/groot_n15/robocasa/steer/pq2/` (pq2_cell_runner.sh·p0_gate.py·make_fit_manifests.py·aggregate_v2.py·p3_lane_*.sh) |
...
- make_fit_manifests.py는 디스크의 전 episode를 층화 split — pq3는 `make_pq3_manifests.py`로 seed 구획 강제.
- succ_ever_th(corrected 채점)는 PPCS(stove) 전용 — drawer/ppcc는 env 원판정.
- 러너 preflight에 token_select·α key·matrix SHA 대조 추가(pq2의 α 오배선·무효 gated 사고 재발 방지).
...
- Codex 협업: `.claude/skills/codex-collab/SKILL.md` — 구현 완료 후 **Gate 2 코드 리뷰**(`scripts/utils/codex_ask.sh review`).
  ...
  `docs/collab/2026-07-13-pq3-gate1.md` (thread 019f5a76-68eb-7d91-9312-2646e78a04cb, R1 10건/R2 6건 요지·수용/기각·사용자 결정 — 계획서 이력 참조).
