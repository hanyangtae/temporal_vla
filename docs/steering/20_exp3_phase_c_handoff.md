# exp3(구 pq3) Phase C 핸드오프 (A·B 완료 → C 수집)

작성: 2026-07-16. 설계 단일 출처는 여전히 **`~/.claude/plans/dynamic-riding-aurora.md` (v9)** +
`docs/steering/19_exp3_execution_handoff.md`. 이 문서는 Phase A/B 실측 결과·동결 상수·사용자
결정·C 이후 절차만 담는다. 충돌 시 계획서 우선. 실행 런북:
`scripts/safe/groot_n15/robocasa/steer/exp3/README.md`.

## 0. 명칭 규약 (사용자 지정)

서버는 **승준(seungjun)** = kimseungjun@166.104.146.37:11112 / **srv50(구 w2)** =
junhyeong@166.104.35.50 / **srv48(구 w3/w1)** = junhyeong@166.104.35.48 / **로컬** 로 부른다
(w1/w2/w3 호칭 금지 — srv50/srv48 사용; 스크립트 파일명도 srv50/srv48 계열로 개명 완료(구 *_w2.sh/*_w3.sh)).
...
- 원장: `docs/collab/2026-07-13-exp3-gate1.md`, `docs/collab/2026-07-15-exp3-gate2.md`.
...
- manifest(plan 단계)는 3 cell 생성 완료:
  `outputs/eval/robocasa/groot_n15/steer_eval_exp3/manifests/{pq3_drawer_left,pq3_drawer_right,pq3_ppcc_bread}/collect_plan.tsv`
...
- **S5 drawer 7-phase 분포** (base 10판, tail seed): 성공(3ep) reach 47%·grasp-handle 27%·
  ...
  phase 병합** 순차 시도. gated 성립 게이트(`exp3_gated_gate.py`) 결과와 함께 Phase D 에서 적용.
...
| 항목 | 값 | 위치 |
|---|---|---|
| noise 시리즈 | collect = 500000 + tsv_cell_index×100000 + i×1000 / eval = 3000000 + tsv_cell_index×100000 + ep×1000 (arm 공유, 클라이언트 intra-episode +1) | `exp3/make_exp3_manifests.py` |
| α grid | Table14 GR00T {0.1,0.3,0.5,0.8,1,1.5,2,3,5,10} — fit 에 `--alphas table14` **명시** (CLI 기본은 exp2(구 pq2) legacy) | `fit_phase_conceptor_n15.py` |
...
| p0 게이트 | 수집 15판 ∧ succ≥3 ∧ fail≥3 (env 원판정), backfill 순서 S15.. | `exp3/p0_gate_exp3.py` |
...
| eval 30 | seen 15 = fit 실사용 seed 첫 15(새 noise) + unseen 15(교집합 0) — freeze 가 동결(sha) | `make_exp3_manifests.py freeze` |
| β sweep | fit seed 재사용, β{0.1,0.3}, 하방 = wins > base−2 생존(−2판이면 탈락), 동률→0.1, red flag = 양 β 모두 −4판(rc6), 생존 0 = rc7 사용자 게이트 | `exp3/beta_decide.py` |
| 판정 | {H1,H2,H3}×{drawer,ppcc} 6 Holm(α=.05, 단측 exact McNemar), null 관문 ±4판, 비재현 CI = paired bootstrap(seed 20260715) 상한 < +0.16, gated N/A 는 gate report 자동 파생 | `exp3/exp3_decision.py`(동결, sha 기록) |
| serve/수집 | CAP 7층 0,2,4,8,10,12,15 · CHUNK_LEN 16 · n-action-steps 5 · max-episode-steps 720 · 캡처 OFF eval = `--no-features`(skip_features chunk 경로) | `exp3/exp3_cell_runner.sh` 등 |
...
1. **fit15 수집** (로컬 GPU 0/1/2, cell당 GPU 1개·serve 2개, **SHIP=1 승준 직송**):
   `exp3/README.md` §C 명령 그대로. SHIPPED.tsv(size+sha ledger)·5MB 상식 체크·원격 실물
   대조가 내장돼 있음(유실 사건 표준). natural reset 강제(ep-meta replay 금지).
   **직송 목적지(실측 확정)**: `SJ_ROOT='~/datasets/temporal_vla_outputs/eval/robocasa/groot_n15/phase_event_pq3/raw_rollouts'` (원격은 구명 phase_event_pq3 유지)
   — 승준 `~/datasets` 가 HDD(sda2, 283G 여유), `~/workspace` 는 **NVMe 라 금지**
   (스크립트 기본값이 workspace 이므로 SJ_ROOT 오버라이드 필수).
2. **C0** (병렬): `bash exp3/exp3_c0_scan.sh` — ppcc 신규 2종(빈도 기준, potato·bread 제외,
   60+ seed). 완료 후 ① `configs/robocasa/exp3_ppcc_new_cells.tsv` 생성됨 ② `queue/queue_lib.sh`
   CELLS 에 objA/objB 행 기입 ③ 신규 tsv 로 plan 생성.
3. cell별 `p0_gate_exp3.py` → rc2 면 BACKFILL_EPLIST 로 추가 수집(B≤20 주의), rc3 = cell 탈락
   (3연속 탈락 → **Gate C 4-cell fallback 사용자 게이트**).
4. 게이트 통과 시 `make_exp3_manifests.py freeze` (--pkl-prefix 는 승준 절대경로:
...
5. 이후 Phase D (승준, anaconda python, 스레드 cap): Stage1 sweep → **사용자 게이트** →
   ...
   Phase F: `aggregate_exp3.py` (+confound-audit 필수).
...
- exp2 fit raw 유실 사건(07-16 공지) 관련: exp3 는 심링크 서브셋 없음(manifest 직접 참조),
