...
**그 전제를 검증하는 상위 진단**이다. exp3(구 pq3)와 병행 시 충돌 회피 규칙은 §6.

## 0. 목적 — 왜 이 실험이 conceptor 변형보다 먼저인가

exp2(구 pq2) 재설계 라운드는 위약 대조 포함 null로 종결됐고(`docs/collab/2026-07-10-steering-redesign-gate1.md`
...
1. **재현성 전제는 이미 확증됨**: exp3 Phase B **S4 natural-reset 재현성** — 동일 seed·noise
   → 전 필드 동일 (`docs/steering/20_exp3_phase_c_handoff.md` §2). 따라서 실패 episode를 같은
...
3. **Donor 데이터 가용성** (`docs/steering/NOTICE_exp2_fit_loss_for_exp3.txt` 필독):
   - 생존: 구 3-scene cell(bread/potato/apple) fit 원료 **180판, 승준 datasets에 실물 확인됨**.
   - 유실: exp2 seed-변형 5 cell fit 원료. 필요 시 (scenario_seed, inference_seed) 결정적
     재수집 가능하나 **보류 결정(사용자)** — 이 실험은 생존 cell 또는 exp3 신규 수집분으로 시작.
   - exp3 수집분 pkl에는 env-step GT(`env_step_phases/env_step_success`)가 자동 포함(커밋 ea61d24).
...
5. **서버 호칭**: 승준 = kimseungjun@166.104.146.37:11112 / srv50(구 w2) / srv48(구 w3/w1) / 로컬
...
- **Donor**: 같은 cell 성공 episode의 저장 activation 시퀀스(승준 생존 180판 또는 exp3 수집분).
...
2. **placebo-fail**: 다른 **실패** episode의 activation을 동일 방식 주입 — "아무 교란이나
   결과를 흔드는가"의 방향성 통제 (exp2에서 위약이 최대 양성과 동급이었던 교훈).
...
- 판정: 실패→성공 **구제율**, 같은 seed paired 비교, exact McNemar (exp2/exp3 판정 도구 재사용
  가능: `scripts/safe/groot_n15/robocasa/steer/exp3/exp3_decision.py`의 검정 부분 참조).
...
2. **cell·데이터 확정** (사용자 게이트): 대상 cell, donor 출처(승준 생존분 vs exp3 수집분),
   GPU/시점(§6).
...
4. **patch hook 구현 + 유닛** (§4): exp3와 파일 분리(신규 파일), 브랜치 `exp/patching-ceiling`
   (exp3 브랜치 `exp/pq3-coast-align` 수정 금지).
5. **스모크**: 결정론 재확인 n=3 (no-patch 재실행 = 원 결과 일치) → sham patch(donor=자기 자신)
   = base와 완전 일치 확인 (exp3 S2b sham 패턴 재사용).
...
- `scripts/serve/steering_hooks.py`: `ConceptorSteering.__init__/register`가 hook 배선의 기준.
  ...
  exp3 Per-Step 훅의 `reset_step_counter()`/step 카운트 패턴 재사용(요청마다 K회 발화).
...
- token 선택: v1은 `token_select=all`(exp3 표준, 49-token 정렬). donor 저장 형식과 일치 필수.
...
- 로컬 GPU는 exp3 eval이, 승준은 exp3 fit·아카이브가 점유 — **실행 시점과 GPU 배정은 사용자
  게이트**. exp3 유휴 구간(수집 완료~판정 대기)에 끼워 넣는 것을 권장.
- 디스크: patching rollout은 캡처 OFF라 mp4·TSV 수준. donor NPZ는 소용량만 로컬 회수.
  승준 HDD 여유는 exp3 full-token 아카이브(~80GB 예정, doc 20 §2)가 우선.
...
2. donor 출처: 승준 생존 180판(bread/potato/apple) vs exp3 신규 수집분(drawer/ppcc 신규).
3. 실행 시점·GPU 배정 (exp3 일정과 조율).
