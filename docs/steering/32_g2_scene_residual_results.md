...
3. **~~★발견 A — 모델은 scene 을 "외운다"~~ (2026-07-30 하향)**: 어떤 제거(선형 rank19·SAE selective 35)도
   ...
   **새 scene 의 방향은 기존 scene 들이 span 하는 공간 밖**이다. scene 성분은 조합적
   속성(레이아웃 요소·재질)이 아니라 scene 별 고유 방향으로 보이고, 이 사실이 이후 모든 설계를
   제약한다(→ §5 온라인 잔차화 포기).

   > **⚠ 정정 (2026-07-30) — "모델이 scene 을 외운다"는 이 실험으로 말할 수 없다.**
   > 제거 rank 는 §1 에 적힌 대로 **20 클래스 이론 한계로 ≤19** 이고 SAE selective 도 35 개다.
   > 1024 차원에서 19~35 차원을 걷어내는데 새 scene 의 방향이 살아남는 것은 거의 **산술적으로
   > 보장**된다. 게다가 §3-3 이 스스로 "새 scene 의 고유 방향을 **원리적으로 못 담는다**"고
   > 적고 있다 — 원리적으로 불가능한 일을 못 했다는 사실은 암기의 증거가 될 수 없다(순환).
   > "모델이 scene 을 암기한다"와 "scene 부분공간이 19 차원보다 훨씬 크다"는 **같은 결과를
   > 예측**하며 이 실험은 둘을 구분하지 못한다.
   >
   > **살아남는 소득**: 본 scene 에서 scene 성분을 지워도(0.812→0.162) outcome read 가 유지된다
   > → **outcome ≠ scene**. 이게 G2 의 실질 결과이고 유효하다.
   >
   > **온라인 잔차화 포기 결론도 유지**되지만 이유가 다르다. "모델이 외웠기 때문"이 아니라
   > **"train scene 으로 추정한 부분공간은 새 scene 에 전이되지 않는다"**(rank 상한 포함) —
   > 암기 여부와 무관하게 성립하는 제약이다.
...
## 4.9 G3 연산자 사전 등록 결정표 (exp5-1_next_computations.txt §4 — 결과 확인 전 등록)

연산자 가족: setM(setpoint형) / 고정 스텝 h−αr̂ / 성분 제거 h−(h·r̂)r̂ / 부분공간 M=I−Σr̂ᵢr̂ᵢᵀ.

| 계산 B 결과 | β sweep 결과 | 선택 |
|---|---|---|
| 1차원 | 패턴 A(용량 과다) | **성분 제거** 우선, 고정 스텝 차선 |
| 1차원 | 패턴 B/C(방향 무익) | 개입 계열 종결 보고 |
| 부분공간 | — | 부분공간 연산자 (다중 r̂ 또는 SAE feature 집합 부활) |

어느 경우든 **setM(setpoint형)은 기본 탈락** — s 추정 부실(신뢰도 0.708·노이즈 33.7%)
+ 표적 scene 라벨된 성공판 요구(배포 일반성 없음).

**armO 추가 (2026-07-27 사용자 검토 반영 — 성분 제거의 산수 결함 정정과 함께)**:
성분 제거 h−(h·r̂)r̂ 는 실측 좌표(p_succ 12.7, gap 15.9)에서 실패 판을 **1.8 gap** 이동
(setM β=1.0의 파국 조건보다 큼) + 성공 판을 이유 없이 0.8 gap 이동 + s=−64.7 토큰에선
4 gap — **보존율 붕괴(exp5-3 해악 52/55의 실체)를 악화시키는 구조**라 후보 강등.
대안 **armO(단방향 문턱)**: h′ = h − γ·max(0, (h·r̂)−s)·r̂ — 성공 쪽(p≤s)은 identity,
실패 쪽만 목표를 넘지 않게 회귀. s를 목표가 아니라 **문턱**으로 쓰므로 추정 노이즈에
덜 민감. 결정표의 "1차원 & 패턴 A" 칸 1순위를 성분 제거 → **armO**로 교체(성분 제거는
탈락, 고정 스텝은 차선 유지). ⚠ serve 배선: SetpointSteering 은 양방향 오차비례라
max(0,·) 게이트 추가 필요(소규모 hook 수정).

**정직성 각주**: 계산 B의 1회 반복분(잔차 read 0.480 = 1차원 시사, §4.8-[4-2])은 이 표
등록 **전에** 관측됐다. 등록 후 산출분은 반복 2~4차 + 각 단계 순열 null z (g3_precalc).

**G3 착수 전 체크리스트 (4개 미충족 시 eval 금지)**:
- [ ] 연산자 — 계산 B(반복판) + 위 결정표
- [ ] 용량 — exp5-3 β sweep
- [ ] 개입 범위 — 계산 C (어느 토큰; G1의 "scene은 future에" + 잠정 단서 "outcome은
      action-agg 0.845 > future-agg 0.800"이 갈리면 "future 잔차화·action만 밀기" 설계)
- [ ] 개입 시점 — 후반 개입 arm 공백(분리도 t=0 0.73→t=37 0.86 증가; phase-gated 배선
      기존재, 단 proximity-phases는 특권 정보라 "알면 밀리나" 판정용)
- 검정력: 전체 SR 금지 — (scene,seed) 셀 paired 구제율/보존율 분리 (exp5-3 해악 52/55는
      보존율 붕괴; 구제율은 별도)

## 4.10 layer×task 차원 지도 (g3_precalc 전 격자, 07-27 심야)

held-out scene 방향 포착률(비퇴화 지표, future agg) + layer 간 r̂ cos + 세그먼트:

| cell·L | k1 | k4 | B판정 | seg AUROC (state/fut/act) | seg cos |
|---|---|---|---|---|---|
| drawer L0 | 0.50 | 0.58 | 1축 | 0.62/0.82/0.80 | 0.16/0.41/0.41 |
| drawer L2 | 0.32 | 0.57 | 부분공간 | 0.69/0.74/0.80 | 0.20/0.27/0.42 |
| drawer L8~12 | 0.42~0.43 | 0.54~0.56 | 부분공간 | act 최고 0.83~0.85 | act 0.44~0.45 |
| mixer L0~12 | 0.04~0.17 | 0.32~0.39 | 부분공간(약) | **state 최고 0.71~0.73** | state 0.19~0.23 |

- **layer 간 r̂ cos**: 두 task 공통 — L8·10·12 서로 0.73~0.94(같은 축의 깊이 전파),
  **L0 은 깊은 층과 거의 직교**(0.05~0.25, 독립 신호), L2 중간. → 깊은 층은 1개(L12)로
  충분, L0 축은 별도 후보.
- **task 대비**: drawer 는 "지배적 1축(~50%) + 약한 잔여 부분공간", mixer 는 공통축 자체가
  약함(scene 별 방향 cos 0.01~0.09) — **1축 서사는 drawer 한정**. G3 1차 대상 = drawer.
- **beer (07-28 추가, 창 [0,44), 5층 전수)**: **창 내 무신호** — read AUROC 0.60~0.63 에
  null_z 1.4~1.9(전 층 z<3), 방향 cos 0.02~0.04, 세그먼트 전부 미달(no_segment_signal).
  exp5-3 §6 진단(결판 phase 진입 중앙 record 47 = 창 밖 + 실패 모드 이질)과 정합 — 보류 정당.
  **3-task 유형론**: drawer=강한 공통축 / mixer=약한 분산 신호 / beer=창 내 무신호.
  조작이 길고 복잡할수록 공통 창(최단 성공 길이)이 결판 장면을 못 담는 구조적 기울기 —
  긴 task 는 이벤트-앵커 창(exp5-3 §6 설계) 없이는 이 분석 계열 자체가 불가.
- **세그먼트가 task 마다 다름**: drawer=action 토큰, mixer=state 토큰이 최강 — "어느 토큰을
  미나"도 cell 별 재선정 대상.
- **결정표 갱신(사전 등록 규칙 적용)**: 계산 B 최종 = **부분공간(경계)** (drawer L2-12
  subspace_capture, 증분 ≥0.1) → 연산자 후보 = 단일 r̂ armO 가 아니라 **소수 다중 r̂(k=2~4)
  armO 확장** 또는 top-k 부분공간 사영형. 단 k1 이 절반을 담으므로 "k=1 armO vs k=3 armO"
  두 arm 비교가 자연 설계. mixer 는 연산자 이전에 표현 근거 부족으로 보류.
