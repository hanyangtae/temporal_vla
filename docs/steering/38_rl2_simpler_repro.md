...
**③ 게이팅의 원료인 "성공 상태 해악"이 우리 재현에서 소멸.** 논문에서 adaptive 이득은
always가 rephrase보다 **나쁜** task(tape −7.4, dino −3.3)를 게이트가 구제하는 데서 나온다.
우리 실측에선 always가 어느 task에서도 안 나쁨(최악 −0.7) → 게이트가 구할 해악 자체가 없어
adaptive ≤ always가 필연. 즉 **scaling-law 전제(성공 상태 개입=해악)가 우리 플랫폼에서
재현되지 않았고**, 그러면 게이팅 이득도 없다.
