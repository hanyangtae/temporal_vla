...
## ★ 원인 규명 (08-07, 3-agent 심층조사)

adaptive −7.0pp의 원인을 코드 diff / SAFE 게이트 실측 / ckpt 출처 3축으로 조사한 결과.

### 배선·ckpt는 원인이 아니다 (전부 실측 배제)

- **코드**: 플래그·기본값·루프·adaptive 분기 전부 저자와 등가. 유일한 차이는 집계 —
  저자 프로토콜은 **(seed×task) 셀별 α max** (`summarize_logs.py`의 `best=max(valid)`).
  정정 시 adaptive 46.7→**48.2**, 논문(Table III **53.8±3.2**) 대비 **−5.6pp 잔존**.
- **ckpt**: pi0·QAM·CoVer는 리비전 단위로 논문과 확정 동일(QAM은 Table V 13항목 전수 일치).
- **CP band 커버리지도 유효**: 성공 rollout의 band 초과율/명목 α 비 = 0.88~1.65(평균 1.2)
  → "우리 플랫폼에서 band가 깨졌다" 가설 **기각**.
- **시간-검출기 가설도 기각**: 성공 내부 지속시간 AUROC 0.42~0.49, t-잔차화 후에도 0.585 유지.

### 실제 원인 — 세 겹의 합성

**① 검출기가 이 OOD 4 task에서 거의 무력하다.** fixed-t AUROC 0.516(t=0)~0.62(천장),
실패 에피소드의 **36.5%는 끝까지 미검출**(tape_measure는 88% 미검출). SAFE 학습은 IID 4 task
뿐이라 eval 4종은 전부 unseen — 절대 임계값을 unseen task에 그대로 적용하는 구성.
공개 SAFE ckpt는 저자 클라우드 rollout(비공개, 2026-05-16)으로 학습된 것이고 README가
**"retrain on your platform" 2회 경고** — 저자 스스로 인지한 취약 지점.

**② 게이트에 latch가 없다 — adaptive는 전환이 아니라 듀티사이클 혼합.** 발동은 4-step
chunk 단위 토글(매 timestep prefail 재초기화, `run_simpler_eval_with_openpi.py:338`)이라
adaptive = **rephrase 80% + always 20% 혼합**으로 동작. 혼합 예측 SR 0.458 vs 실측 0.444 —
오경보 오배분만큼 아래. 선택성(fail:succ 발동비 2.4, precision lift 1.44배)으로는 always를
구조적으로 못 이긴다. (논문 Algorithm 1도 per-step 판정이므로 코드는 논문에 충실 —
단 Fig 3의 "감지 후 전환" 서사와는 뉘앙스 차이.)

**~~③ 게이팅의 원료인 "성공 상태 해악"이 우리 재현에서 소멸~~ — 철회 (08-07).**
task 평균 ΔSR≈0에서 "해악 없음"으로 건너뛴 추론 비약이었다(사용자 지적). 같은 초기조건
600쌍의 불일치쌍 실측: **합성이 망친 판 89(14.8%) / 구한 판 103(17.2%)** — 성공 상태 해악은
실재하며 평균이 상쇄를 숨기고 있었다. 완벽 게이트의 상한 SR = (183+89+103)/600 = **62.5%**
(단 두 arm의 정책 노이즈 차이로 불일치 일부는 비인과 — 상한 과대추정). 결론 수정:
**원료는 풍부하고, 책임은 온전히 검출 축(①무력 + ②latch 없음)에 있다.** 검출기가 이 분리를
일부라도 잡으면 adaptive가 살 수 있는 구조 → SAFE 재학습의 동기 강화.

### 보너스 발견 — 저자 검출기 지표는 길이 confound에 오염

SAFE 평가 지표(에피소드 max 점수 AUROC)는 실패(38 chunk) vs 성공(~15 chunk)의 max 기회
불균등 덕에 **0.756**으로 보이지만, 동일 창 max로 통제하면 **0.54~0.59** — 거의 우연.
우리 seen18 길이-confound 교훈(time-pooled 분리=아티팩트)이 이들의 검출기에 그대로 적용된다.

### 이전 보고 정정

- "발동 11.2%·t≥116 집중"은 **α=0.1 lane 한정** 관측이었다. 9개 lane pooled 발동률은
  20.0%, 후반 집중은 α≤0.1에서만 (α≥0.2는 band 골짜기가 t=104~140에 있음).
- "band 플랫폼 미스매치 가능성"(중간보고)은 실측으로 **기각**됨.

### 남는 질문과 검증 경로

저자 머신에서 53.8이 나온 메커니즘은 미확인으로 남는다(비공개 rollout으로 학습된 검출기가
저자 플랫폼 분포에서는 선택성이 높았을 가능성이 유력하나 검증 불가). 검증 경로:
1. **SAFE 재학습**(README 레시피, 우리 rollout 1,200ep) — 단 ①만 해결, ②③은 남음.
2. **latch 실험**(발동 시 에피소드 끝까지 composed 유지) — 예측: adaptive→always(47.7) 수렴.
3. **IID suite에서 adaptive 재현** — 검출기가 seen인 조건에서 논문 Table II와 대조.

## ★★ 결말 — SAFE 재학습으로 adaptive 복원 (08-08)

README 권고 경로(우리 플랫폼 rollout 1,200판 수집 → SAFE 재학습 → CP band·α 휴리스틱 재산출)를
전부 밟고 adaptive 재평가 (3 seed × 우리 α top-3 = 9 lane, 1,800판, 전 lane 200/200·에러 0):

| adaptive 구성 | 셀별 α 사후선택 (저자 프로토콜) | α=0.2 고정 (사후선택 없음) |
|---|---|---|
| 번들 SAFE (저자 ckpt) | 48.2 | 44.5 |
| **재학습 SAFE (우리 rollout)** | **52.7** | **49.5** |
| 논문 | 53.8±3.2 | — |
| (비교선) always / rephrase | 47.7 / 45.3 | 동일 |

- **동일 프로토콜 대 동일 프로토콜로 논문 오차범위 안 진입** (52.7 vs 53.8±3.2). 게이팅 이득 복원.
- 기제 = 검출기 품질 회복: 재학습 val ROC-AUC 0.951(번들의 논문값 0.904↑), bal_acc 83.5~88.0
  (번들 검출기를 우리 rollout에 걸면 0.60~0.79).
- **latch 없는 듀티사이클 구조는 무죄** — 검출기가 유능하면 chunk 토글로도 always를 이긴다.
- 재학습 rollout은 IID 4 task뿐인데 OOD eval에서 복원됨 → 결정적이었던 건 task 전이가 아니라
  **플랫폼(점수 분포) 정합**.

정직한 단서: **α 민감도는 심각한 실재** — 같은 seed에서 α 0.15→0.20→0.25가 45.5→**54.0**→44.0
(Δα=0.05에 ±5pp). 셀별 사후선택은 이 요동의 max를 취하므로 노이즈 팽창(+3~4pp 추정) 포함.
사후선택 없는 정직값(49.5)은 always 대비 +1.8pp로 단독 유의하지 않다. 논문도 같은 프로토콜이라
비교는 공정하지만, **게이팅 이득의 상당분이 α 사후선택 프로토콜에 실려 있다.**

### 최종 판정 (재현 사가 종합)

> 3개 baseline arm은 공개물만으로 재현된다. **adaptive(헤드라인)는 공개 ckpt로는 재현되지
> 않으며**, README가 강권하는 플랫폼별 SAFE 재학습을 수행해야 논문 오차범위에 진입한다
> (48.2→52.7). 게이팅 이득은 실재하되 **검출기-플랫폼 정합**과 **α 사후선택 프로토콜**에
> 민감하게 의존한다.

재학습 인프라: `collect_safe_rollouts.sh` · `compute_alpha_heuristic.py`(α 재산출, split 재현
검증 8/8) · `rerun_adaptive_ours.sh`. 재학습 ckpt =
`third_party/SAFE/logs/open_pizero-bridge-lstm-ours_cpTrue/20260807/123421/`.

## 시사점 (우리 연구 관점, 결말 반영)

1. **"언제 개입할까" 축의 실체**: 게이팅 이득은 실재하나 배포 가능한 형태가 아니다 —
   플랫폼마다 rollout 재수집·재보정이 필요하고 α에 ±5pp로 민감. 우리 니치(조기 검출·phase
   라우팅·보정 프리한 게이트)의 개선 여지가 정확히 여기다.
2. **검출기-플랫폼 정합이 game-changer** — 같은 아키텍처·같은 hyperparameter에서 학습 데이터의
   플랫폼만 바꿔도 adaptive가 −7pp↔논문 수준을 오간다. 실패검출기의 scene/domain 특이성에 대한
   우리 관측(exp5 G2, SAFE sim→real)과 같은 현상의 세 번째 독립 사례.
3. **합성(다양성 주입)은 어느 조건에서든 재현** — Stage 2 이식의 기술적 토대 유효.
4. Stage 2 설계 교훈: 게이트 threshold는 **사후선택 없는 고정값을 primary로** 보고하고,
   α 스윕은 민감도 곡선으로 별도 보고할 것.
