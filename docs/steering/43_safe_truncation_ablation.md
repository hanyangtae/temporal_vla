...
예측력 없음 — 그룹핑은 배포 데이터에서 mixed vs pertask 직접 실측이 유일**. 그룹핑
사전판별 시도는 46에서 3라운드로 종결(미확증; S1 잔존 가치는 표현 구조 진단 — steering 입력).
...
### 8) RL2/SAFE 원본 세팅 대조 (2026-08-28; 같은 날 심화 세션 질의응답으로 §8.1 정정)

RL2 심화 세션의 절제 결과 보고 건에 대한 3자 대조. **RL2의 원본 세팅이 무엇인지**를
논문·코드에서 확정하고, 같은 데이터·같은 프로토콜에서 세 조건을 두 지표축으로 재채점했다.
(초판은 "효과 없음 보고 = 측정축 누락"으로 프레이밍했으나, 심화 세션 질의응답 결과
**"효과 없음" 판정은 애초에 없었다** — §8.1 참조. 재채점 표와 지표축 분석 자체는 유효.)
...
**판정 (우리 데이터 한정)**: RL2식 지표(episode-max AUROC·TPR/FPR)에서는 절제 효과가 없고,
절제의 실효과는 조기성(preW +0.23)에만 있다 — §1·§2 결론의 지표축 재확인. RL2식 지표는
(a) 조기성을 원리적으로 측정하지 못하고 (b) full 길이 test에서 length_auroc=1.0인 우리
세팅(실패=timeout)에선 timer도 만점을 받는 길이 지름길 지표다(38 문서 "보너스 발견"의
자기 플랫폼 재확인).

#### 8.1) 심화 세션 질의응답 — 정정과 플랫폼 차이 확정 (2026-08-28)

심화 세션에 직접 질의한 결과, 초판의 전제("절제 효과 없음 보고")가 틀렸다:

- **"효과 없음" 판정은 없었다 — 절제가 no-op이라 학습을 1분 만에 중단**한 것이다.
  SIMPLER SAFE 학습 rollout은 **성공·실패 불문 전부 38 inference step 고정 길이**
  (수집이 성공 순간에 끊지 않고 horizon 완주; 그쪽 로그 `mean=38.00 sd=0.00 → cap=38,
  잘린 판 0/720`). **우리 실측 교차확인**: succ 516판·fail 684판에서 각 40판 표본 —
  전부 len 38 (80/80). 08-26 빈 로그·미완결 wandb는 의도적 kill 잔재이며 완주 산출물이
  없는 게 정상.
- **함의 — 절제 규약의 적용 조건**: 절제는 "실패=timeout, 성공=조기종료"의 **길이-라벨
  비대칭이 있는 데이터에서만 정의되는 질문**이다. SIMPLER 고정-horizon 수집엔 길이 축
  자체가 없어 질문이 성립하지 않는다 (학습 데이터에 길이 confound 원천 부재).
  RoboCasa(우리)와 SIMPLER(RL2 재현)의 결과 차이는 모순이 아니라 **수집 종료 규약 차이**.
- **심화 세션의 실제 지표는 공정 고정시각 AUROC였다** (생존·미발화 판만, 시점별): 재학습
  SAFE, OOD suite 600판 3-seed에서 pooled 0.33~0.48, tape_measure 전 구간 역부호 —
  **길이 confound 없는 데이터에서도 애초에 신호가 없음**. 38 문서 ①(OOD 4 task에서 검출기
  무력, fixed-t 0.516~0.62)의 더 강한 재확인. in-domain 학습→OOD 평가라 근본 무능 vs 전이
  실패는 미분리 (IID suite 재채점으로 갈릴 수 있음, GPU 불요 — 그쪽 후속 후보).
- 그쪽 부가 지적: SIMPLER는 **학습 rollout full-length / eval episode는 성공 시 조기 종료**
  비대칭이라 eval-side max-score 지표엔 길이 지름길이 여전히 남는다 — 우리 세팅에 없는
  그쪽 고유 아티팩트.
