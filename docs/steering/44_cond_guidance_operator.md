...
### 7.3 실패 기전 분석 — 개입은 행동을 바꾸는데 "내용"이 없다

replay 결정성을 이용해 β1.0 실패 판 12개의 base vs 개입 영상 프레임 발산을 측정
(`divergence_check.py`, mean|Δpixel|>2):

- **9/12판에서 물리 발산** — 개입 ON 후 챙크 1개(~5 record) 안에 궤적이 갈라짐.
  즉 발화→phase→margin게이트→h변형→행동 사슬은 인과적으로 완결 작동.
- 3판 무발산: 1판은 개입 0(phase 미방문), 2판은 개입 ON에도 무발산 = margin 게이트가
  record 대부분 no-op (τ 보수성의 dose 불균일).
- **정성(ep1 프레임)**: base는 drawer 앞에서 실패-방황, condg는 개입 후 drawer 접근
  자체를 이탈해 **다른 실패-방황 경로**로 감. 과제-지향 교정(재-grasp)이 아니라
  궤적 교란 — 위약의 효과와 질적으로 동일.
- **판정: read≠write의 물리적 실체** — 활성화 공간의 "실패 방향 성분 제거"가 행동
  공간의 "성공 행동"으로 번역되지 않는다. 개입은 궤적을 다른 basin으로 차는 능력만
  있고 어느 basin인지는 못 정함. OvenRack 상시위약 구제(성공 basin이 가까운 task는
  아무 섭동이나 종종 성공)와 일관 → 재샘플 축의 기전적 근거.
- 미해결 진단 1건: 개입 후 margin(연산자 자가평가)이 실제로 내려갔는지 — eval 활성화
  미캡처라 불가. 차기 라운드에서 1–2판 캡처 켜고 확인 권장.

## 8. oracle_early · resample (사용자 지시 2실험, 2026-08-20 — 4 task)

replay oracle 타이밍: 셀별 알려진 발화 record(TRIGGER_TSV)를 기준으로 클라이언트가 개입.
oracle_early = 발화−3 record 부터 phase-follow(condg d0 β1.0 무게이트, 위약 쌍),
resample = 발화 record 부터 inference seed 재추첨(offset 900000, steering 無).

**oracle_early (drawer-L·drawer-R·Dish)**: 전 task 구제 0, 처치=위약 셀 단위 동일
(drawer-R 위약만 파손 1 차이 = 노이즈). **발화 3 record 선제로도 null — 조기개입 축 종결.**

**resample (4 task)**:

| task | 구제 (실패셀) | 파손 (성공셀) | 파손의 출처 |
|---|---|---|---|
| drawer-L | 1/17 (s3n1) | 1 (s8n0) | FP 재추첨 |
| Dish | 1/24 (s0n5) | 2 (s6n0·s6n5) | FP 재추첨 |
| drawer-R | 1/13 (s0n0) | 3 (s5n0·s4n5·s4n6) | FP 재추첨 (FPR 0.64 대역) |
| candle | 2/11 (s4n0·s9n5) | 1 (s5n6) | FP 재추첨 |
| **합계** | **5/65 (7.7%)** | **7/95 (7.4%)** | |

판정:
1. **캠페인 최초의 결정적-replay 구제**가 4 task 전부에서 발생 — 방향 없는 재추첨이
   선형 연산자 전 계열(구제 0/108+)이 못 한 것을 함. OvenRack 상시위약 소견의 일반화.
2. 단 **1회 추첨의 순효과 ≈ 0** (구제 5 ≈ 파손 7). 구조가 중요: 파손은 전부
   **오발화(FP) 성공셀의 재추첨**에서만 발생 → 파손은 detector FPR에 종속(문턱·α로
   억제 가능), 구제는 재추첨 횟수 K로 1−(1−p)^K 확장 가능.
3. → 프레임 전환: "steering 연산자"가 아니라 **감지-후 재시도 정책**(K회 재샘플 +
   FP 억제)이 시나리오 기준 ②(2–3/10 구제)에 도달할 수 있는 유일한 실증 경로.
   45 기준 채점: ① 파손 7(FP-매개, 억제 여지) — 조건부 미달, ② 7.7% — 미달(K=1),
