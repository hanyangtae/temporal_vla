...
## 0.5 ★ 시나리오가 연산자에 요구하는 범위·성능 (2026-08-18 합의, 상세 = 45 문서)

hotfix 시나리오(사용자 로그로 무학습 패치, 다음 모델 업데이트까지)가 연산자에 실제로
요구하는 것 — 이 이상을 스스로 떠안지 말 것:

- **범위**: fit에 쓴 로그와 **같은 task·같은 환경(같은 주방·같은 물건 집합)까지만.**
  그 안에서 판마다 달라지는 것(로봇 초기 자세·물건 위치의 잔차이·정책의 확률적 샘플링)은
  커버 필수 — "실패의 재발"이 곧 이 변동 안의 사건이므로. 다른 환경·물건·task 일반화는
  요구하지 않음. **환경을 외운 연산자여도 그 환경에서 작동하면 합격.**
- **성능 (우선순위 순)**:
  1. 성공하던 판을 깨지 말 것 (파손률 ≤ 위약) — 구제보다 우선.
  2. 실패하던 판의 **일부**를 꾸준히 뒤집을 것 — 전부 불요, 10판 중 2–3판이면 성립.
...
- **현재 위치**: 1·4 충족, **2·3 미달** — 선형 활성화 개입 전 계열(setM·수축 3종·condg
  × β·시점·layer)이 구제 0·처치=위약 (42·44). 기전(44 §7.3): 개입은 궤적을 바꾸지만
  방향을 못 정함 → 방향 무주장 개입(재샘플)이 차기 1순위.
