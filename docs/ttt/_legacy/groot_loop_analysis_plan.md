...
| lxml 설치 | ✅ | robocasa 의존 |
| GR00T 모델 다운로드 | ✅ | `nvidia/GR00T-N1.6-3B` → `/cache/checkpoints/nvidia/GR00T-N1.6-3B` |
...
> 반드시 로컬 경로 (`/cache/checkpoints/nvidia/GR00T-N1.6-3B`) 사용.
...
- **model_path**: HF hub ID (`nvidia/GR00T-N1.6-3B`) 사용 시 rollout_policy.py에서 `split('/')[-3]` IndexError. 로컬 경로 `/cache/checkpoints/nvidia/GR00T-N1.6-3B` 사용.
