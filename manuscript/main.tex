...
% 컴파일: XeLaTeX (Overleaf 는 Menu > Compiler 에서 XeLaTeX 선택)
...
\usepackage{iftex}
\usepackage{kotex}
% 글꼴·장평·자간은 XeLaTeX 에서만 적용한다 (Overleaf: Menu > Compiler > XeLaTeX).
% pdfLaTeX 로 돌리면 아래 블록을 건너뛰고 kotex 기본 글꼴로 조판된다.
\ifXeTeX
  \XeTeXlinebreaklocale ""
  \IfFontExistsTF{lmroman10-regular.otf}{%
    \setmainfont[FakeStretch=0.9,LetterSpace=-5]{lmroman10-regular.otf}%
      [BoldFont=lmroman10-bold.otf,ItalicFont=lmroman10-italic.otf]}{%
    \defaultfontfeatures{FakeStretch=0.9,LetterSpace=-5}}
  \IfFontExistsTF{UnBatang.ttf}{%
    \setmainhangulfont[FakeStretch=0.9,LetterSpace=-7]{UnBatang.ttf}%
      [BoldFont=UnBatangBold.ttf]}{%
    \defaulthangulfontfeatures{FakeStretch=0.9,LetterSpace=-7}}
\fi

% 그림이 뒤쪽 별도 쪽으로 밀리지 않도록 float 허용치를 넓힌다
...
\usepackage{graphicx}
\usepackage{dblfloatfix}   % 두 단 걸침 그림(figure*)이 뒤쪽 별도 쪽으로 밀리는 문제 교정
\usepackage{amsmath}
\usepackage{setspace}
...
\twocolumn[{
\begin{center}\ifXeTeX\addfontfeatures{FakeStretch=1,LetterSpace=0}\addhangulfontfeatures{FakeStretch=1,LetterSpace=0}\fi
...
% 두 단 그림은 다음 페이지부터 배치되므로 본문보다 먼저 선언한다.
% Figure 2 대기로 Figure 3도 문서 끝까지 밀리지 않도록 함께 앞에 둔다.
\begin{figure*}[!t]
  \centering
...
\textbf{Cluster가 바뀌는 시점은 압축 모델에 관계없이 일관된다.} 같은 activation을 AE와 SAE로 각각 압축·clustering한 뒤, 앞 스텝과 cluster가 달라지는 지점(전환)을 $\pm$1스텝 이내에서 짝지어 일치도를 F1 score로 쟀다. 두 모델의 전환 일치 F1은 0.66$\sim$0.75로, 같은 개수의 전환을 무작위 위치에 놓았을 때의 0.51$\sim$0.53을 웃돌아 전환 시점이 압축 모델이 아니라 activation 자체에 담긴 구조임을 보인다.
