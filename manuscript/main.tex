...
\end{center}
\begin{quote}\small
Vision-Language-Action(VLA) 모델이 작업을 수행할 때 스스로 현재 어떤 작업 단계를 진행중인지를 우리가 알 수 있다면, 이는 VLA모델에 대한 설명가능성과 추론 시 제어 및 개입의 출발점이 된다. 본 연구는 VLA의 내부 activation을 통해 모델이 스스로 인식하는 현재 작업 단계를 직접 읽는다. Gr00t N1.5의 action head(DiT)의 residual stream activation을 라벨 없이 clustering하면, 사람이 정의한 primitive action phase보다 더 세밀한 action phase cluster 이 생긴다. 이때 한 action phase cluster는 특정 primitive action phase에 국한되어 나타나며 다른 primitive action phase에서는 잘 나타나지 않는 특성을 관찰했으며(순도 0.80), 단순 시간 진행도 대조군보다 1.67 bits 높은 단계 정보를 담는다. 이 구조는 데이터 처리 과정을 바꾸어도(AE$\rightarrow$SAE) 같은 시점에서 phase전환을 유지하고, 다양한 task,scene에서도 유지된다(z +8.8$\sim$+13.5). 즉, VLA activation에는 세밀하지만 정합적인 action phase 구조가 존재하며 온라인 판독에 이를 활용 할 수 있으며, 본 논문에서 이를 실험적으로 확인하였다.
\end{quote}
\vspace{1.5ex}
}]
...
\begin{figure*}[!t]
  ...
  \caption{(a) 군집$\times$GT phase contingency(PPCC candle, k=8): 행을 최빈 phase로
...
\begin{figure}[!t]
  ...
  \caption{(a) 발견 상태의 구간 길이는 GT phase보다 짧다. (b) 시간 대조군 대비 MI
  margin --- 고정 k KMeans가 자율 k 방식보다 높다(데이터셋 A, k=24).}
...
\section{방 법}
\textbf{데이터·특징.} RoboCasa 주방 조작 환경에서 GR00T N1.5의 rollout을 수집한다.
대상은 5개 task(OpenDrawer, PnPCounterToCab, OvenRack, DishwasherRack,
CoffeeSetupMug)에 속하는 10개 instruction이다 --- 같은 task라도 서랍의 좌·우나 옮기는
물체가 달라지면 다른 instruction이 되며, 분석은 이 instruction 단위로 수행한다. 각
rollout의 매 스텝에서 action head(DiT)의 residual stream activation을 캡처한다(layer
12, denoising step 3, 49개 토큰 평균 → 스텝당 1536차원). 사람이 주석한 primitive
action phase 라벨(instruction당 3$\sim$6단계)은 학습에 쓰지 않고 평가에만 쓴다. 두
데이터셋을 사용한다: (A) 23 에피소드(phase 경계 91개), (B) 독립 재수집 930
에피소드(10 instruction, 89{,}766 스텝; 이 중 1개 instruction은 30 에피소드 소표본).

\textbf{군집화.} raw activation(1536차원)을 그대로 오토인코더(AE, latent 16)로 압축한
뒤 KMeans로 스텝 단위 군집화한다(instruction별 분석은 k=8, 전체 통합 분석은 k=24).
PCA 등 사전 차원축소는 쓰지 않는다 --- 예비 비교에서 PCA-64 압축은 이득이 없었고,
특히 축별 whitening은 저분산 노이즈 축을 증폭해 phase 정보를 오히려 없앴다. 군집
중심은 train split에서만 만들고 평가 split의 스텝은 k-NN(k=15)으로 라벨만 옮긴다 ---
재군집화가 필요 없으므로 추론 중 스텝 단위 online 판독이 가능하다.

\textbf{시간 대조군.} activation이 phase 정보를 담는지 재려면, 먼저 ``시간만 알아도
맞힐 수 있는 몫''을 빼야 한다. 조작 task의 phase는 대체로 순서대로 진행되므로
activation을 전혀 보지 않고 경과 시간만 보는 판독기도 phase를 어느 정도 맞힌다. 이를
\textbf{시간 대조군}(clock)으로 정의한다: 에피소드 내 상대 진행도 $t/T$를 발견된 군집
수와 같은 개수의 구간으로 등분위 분할해 얻은 상태열이다. 상태 수를 맞추는 이유는 상태
수가 많을수록 상호정보량(MI)이 자동으로 커지기 때문이며, 이렇게 해야 같은 해상도에서
비교된다.

\textbf{지표.} 군집열이 담는 phase 정보는 MI(군집; GT phase)로 재고, 판정 기준은 시간
대조군을 뺀 \textbf{margin} $= I(\text{군집};\text{GT}) - I(\text{clock};\text{GT})$을
쓴다. margin은 activation이 단순 시간 정보를 넘어 추가로 담고 있는 phase 정보량[bits]을
뜻한다. 각 군집이 특정 phase에 얼마나 국한되는지는 순도(purity)로, phase 경계와의
정렬은 전환 위치를 에피소드 내에서 무작위로 흩은 300회 대비 z-score로 잰다.

\section{결 과}
\textbf{activation phase cluster는 사람이 정의한 phase보다 세밀하다(그림 2a).} 발견된
...
+1.3$\sim$+4.9). 즉 activation은 GT phase를 그대로 재현하는 것이 아니라 그보다 잘게
쪼갠다. 세밀함의 배수는 군집 수 k에 따라 달라지지만, 세밀해진다는 사실 자체는 k를 GT
phase 수(3$\sim$6)에 맞춰도 남는다: k=8에서도 구간 길이는 GT보다 짧게
유지되고(10개 중 9개 instruction), 에피소드당 전환 수가 상태를 한 번씩만 지나는 단조
통과의 상한(k$-$1=7회)을 넘는 instruction이 절반이다(최대 22회) --- 상태가 재방문되는
구조로, k를 키워서 만들 수 있는 모양이 아니다. 반대로 한 instruction(OvenRack)은 같은
k=8에서 GT보다 굵게 병합되어, 해상도는 k가 아니라 activation이 정한다.
...
비율(off-phase rate)의 군집별 중앙값은 instruction 중앙값 0.07이며, 순수한
instruction에서는 0.002까지 내려간다(넓게 걸치는 것도 있어 최대 0.45 ---
instruction별 편차가 있다). 데이터셋 B에서도 순도 중앙값은 0.88로 유지된다. phase
하나는 평균 2$\sim$3개 군집으로 세분되어(phase당 군집 수 1.3$\sim$2.7),
...
4개였다(동급 1, 열세 1). 이는 탐색적 관찰이나, 세밀 단위가 사람 정의 phase의 단순
잡음이 아님을 시사한다.
...
\section{한계와 결론}
GT 라벨 두 벌이 같은 주석 체계의 두 버전이라 ``어떤 phase 정의와도 어긋난다''고는 할
수 없고, 전환 경계가 GT와 정렬되지 않으므로 판독 근거는 군집 정체성(MI·순도)이지 경계
검출이 아니다. 길이 1의 짧은 상태가 구간의 31$\sim$37\%를 차지하며 의미는 미검증이다.
...
단위로 읽을 수 있으며, 이는 VLA 행동의 설명 신호이자 단계 조건부 개입과 실패
감지의 조건 신호가 된다.
...
Semi-MDPs,'' \textit{Artificial Intelligence}, 112:181--211, 1999.
\end{thebibliography}
