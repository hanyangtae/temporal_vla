...
\begin{figure}[!t]
  ...
  \caption{\new{clustering 개수 $k$값 변동에 대한 강건성. 사람 phase 수(3--6, 회색 대역)를 넘는  $k \ge 8$에서 성능이 평평해진다(정확도 0.86--0.89). 옅은 선 = instruction 10개.}}
  \label{fig:ksweep}
\end{figure}
\section{서 론}
...
\new{\textbf{판독 프로토콜.} ``읽을 수 있는가''는 학습에 쓰지 않은 scene(unseen scene)에서의 정확도로 잰다. scene 단위로
...
비교 대상은 (i) 다수 클래스, (ii) \textbf{시간 대조군} --- activation을 보지 않고
경과 시간만 쓴다(에피소드 길이를 모르는 온라인 상황을 위해 절대 스텝 $t$ 기준, 진행도
$t/T$ 기준 둘 다), (iii) \textbf{행동 대조군} --- 정책이 실제로 낸 7차원 행동을 record별
평균·표준편차·마지막값과 에피소드 시작부터의 누적합(팔 위치의 대리값)으로 요약한 28차원.
이는 정책 바깥에서 관찰 가능한 정보만 쓰는 조건이다. (iv) 행동+시간을 합친 가장 강한
외부 대조군, (v) activation 비지도 군집 + 최빈 phase 매핑(라벨은 이 매핑에만 쓰인다),
(vi) activation 지도 multinomial logistic probe이다. (iii)--(vi)은 같은 군집·probe 절차를
쓰므로 입력만 다르다.}
...
\begin{figure*}[!t]
  ...
  instruction 10개). 회색은 외부에서 관찰 가능한 정보만 쓰는 대조군(시간·정책 행동·둘의
  결합), 파랑은 내부 activation. (b) 같은 지도 조건에서 instruction 별 비교 --- 대각선
  위가 activation 승(9/10).}}
  \label{fig:readout}
\end{figure*}
\section{결 과}
\new{\textbf{activation 한 스텝만으로 현재 phase를 읽을 수 있다(그림 3).} 한 번도 보지
않은 scene에서 잰 판독 정확도(instruction 중앙값)는 activation 비지도 군집 0.86, 지도
probe 0.89이다. 대조군은 다수 클래스 0.56, 시간만 0.61--0.68, 정책 행동만 0.61이고, 밖에서
볼 수 있는 정보를 모두 합친 가장 강한 외부 대조군(행동+시간)이 0.77이다. macro-F1로 봐도 0.72(probe) 대 0.56
(행동+시간)으로 우위가 유지된다. 같은 지도 조건에서 비교하면 activation probe가 외부 대조군을 10개 instruction 중
9개에서 이긴다(예외는 OpenDrawer left, $-$0.09). 즉 ``시계만 봐도''도, ``로봇이 무슨 행동을
내는지 보면''도 내부 표상만큼은 알려주지 못한다. 특히 비지도 군집만으로 0.86이 나온다는
점이 중요하다 --- 구조 자체는 라벨 없이 activation에서 나왔고 라벨은 각 군집에 이름을
붙이는 데만 쓰였다. 군집 중심을 train에서 만들고 평가 스텝은 최근접 배정으로 옮기므로,
재군집화 없이 추론 중 스텝 단위로 그대로 돌릴 수 있다.}

\textbf{발견된 단위는 사람이 정의한 phase보다 잘게 나뉜다.} 발견된 상태의 평균 구간
길이는 GT phase보다 짧고(k=8에서 10개 instruction 전부), 에피소드당 전환 수도 상태를 한
번씩만 지나는 상한($k-1$회)을 모두 넘는다 --- 같은 상태를 여러 번 다시 방문한다는 뜻이다.
전환 시점 자체는 GT 경계와 정렬되지 않으나(z $-$1.0$\sim$+0.5), GT phase 단위로 병합하면
정렬이 유의해진다(z +1.3$\sim$+4.9). \new{다만 이 세분이 의미 있는 하위 구조인지는 본
연구의 범위를 넘는다. 판독 성능은 $k$가 phase 수를 넘기만 하면 평평하므로
(그림~\ref{fig:ksweep}), 앞의 결과는 특정 $k$ 선택에 기대지 않는다.}

\textbf{각 군집은 특정 단계에 국한된다(그림 1a).} 한 단계에서 나타난 군집은
...
판독 평가는 scene 단위로 나눴을 뿐 instruction 간 전이는 다루지 않았다.}

\new{결론: VLA의 내부 activation만으로 현재 action phase를 읽을 수 있다 --- 처음 보는
scene에서 정확도 0.86(비지도)/0.89(probe)이며, 시간·정책 행동 등 외부 관찰 정보를 쓰는
대조군(최대 0.77)을 웃돈다. 이 구조는 라벨 없이 얻어지고 scene 성분을 제거해도 유지되며 모델·데이터를 바꿔도
