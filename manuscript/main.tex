...
phase 수(3$\sim$6)에 맞춰도 남는다: k=8에서도 구간 길이는 10개 instruction 전부에서 GT보다
짧고(예: PPCC jug 4.2 vs 53.5 스텝), 에피소드당 전환 수도 모두 상태를 한 번씩만 지나는
단조 통과의 상한(k$-$1=7회)을 넘는다(7.3$\sim$30.0회) --- 상태가 재방문되는 구조로, k를
키워서 만들 수 있는 모양이 아니다. 반대로 OvenRack은 GT와 거의 같은 해상도로
병합되어(8.5 vs 8.5 스텝), 해상도는 k가 아니라 activation이 정한다.
