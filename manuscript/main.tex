...
비율(off-phase rate)의 군집별 중앙값은 instruction 중앙값 0.22이며, 순수한 instruction에서는
0.001$\sim$0.06까지 내려간다(넓게 걸치는 것도 있어 최대 0.47 --- instruction별 편차가
...
정보도 섞여 있다(instruction별 mi\_scene 0.2$\sim$1.0 bits). 그러나 instruction 내 scene별 평균을
제거한 잔차에서 장면 정보는 대부분 사라지는 반면(mi\_scene 중앙값
0.54$\rightarrow$0.24 bits) phase margin은 소표본 퇴화 1개를 제외한 9개 instruction
전부에서 양수로 유지된다(중앙값 +0.34$\rightarrow$+0.28; 3개는 오히려 상승).
