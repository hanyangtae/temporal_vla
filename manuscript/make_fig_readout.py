#!/usr/bin/env python
"""Figure: unseen-scene phase readout accuracy (KAI-2026).
...
def main() -> None:
    ...
    # ---- (b) external baseline vs activation cluster, per instruction
    if len(esae) >= 5:      # instruction 이 충분히 모이기 전엔 (b)는 clock 유지
        x = np.array([esae.get(r["instruction"], np.nan) for r in rows])
        xlabel = "behavioral event accuracy"
    else:
        x = np.array([float(r["causal_time_acc"]) for r in rows])
        xlabel = "time (clock) accuracy"
    y = np.array([float(r["cluster_acc"]) for r in rows])
    good = ~np.isnan(x)
    ax2.plot([0, 1], [0, 1], color="#999999", lw=0.8, ls="--", zorder=1)
    ax2.scatter(x[good], y[good], s=26, color="#4C72B0", edgecolor="black",
                linewidth=0.5, zorder=3)
    # 라벨이 몰리는 우상단을 피해 x 순서대로 위/아래 번갈아 배치
    idx_good = np.where(good)[0]
    for rank, idx in enumerate(idx_good[np.argsort(x[good])]):
    ...
    lo = min(x[good].min(), y[good].min()) - 0.06
    ax2.set_xlim(lo, 1.02)
    ax2.set_ylim(lo, 1.02)
    ax2.set_xlabel(xlabel)
    ax2.set_ylabel("activation cluster accuracy")
    n_win = int((y[good] > x[good]).sum())
    ax2.set_title(f"(b) same protocol, per instruction ({n_win}/{int(good.sum())})",
                  pad=4)
    ...
    print("activation cluster > external:", n_win, "/", int(good.sum()))
