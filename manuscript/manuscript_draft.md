...
된다. 본 연구는 그 표상을 외부 관측이 아닌 내부 활성화에서 직접 읽는다: VLA가 로봇
조작을 수행하는 동안 내부 활성화가 어떤 시간 구조를 갖는지 조사한다. GR00T N1.5의 action head(DiT) 잔차 활성화를 라벨 없이
...
블랙박스로 동작하는 VLA 모델이 자율적으로 로봇 조작을 수행할수록, 모델이 **지금 무슨
작업을 진행 중인지**를 사람이 파악할 수 있어야 한다는 설명가능성(XAI) 요구가 커진다.
그 설명의 자연스러운 단위는 reach → grasp → transport 같은 시간적으로 확장된
primitive 행동 단계(phase)다[9]. 단계를 파악하는 한 방법은 카메라 영상이나 궤적 같은
외부 관측을 분절하는 것이지만[8], 외부 관측이 알려주는 것은 로봇이 **겉보기에** 무엇을
하고 있는가까지다. 모델이 **스스로 무엇을 하고 있다고 생각하는가** — 즉 내부 표상이
가리키는 진행 단계 — 는 내부 활성화를 직접 읽어야만 알 수 있고, 겉보기 행동과 내부
표상이 어긋나는 순간(오인, 실패 전조)을 잡을 수 있는 것도 이 경로뿐이다. VLA 내부
표현을 해석하려는 최근 시도들은 희소 오토인코더(SAE)로 residual stream에서 파지·이송
같은 운동 primitive에 대응하는 feature를 찾아내거나[3,5], 선형 probe로 층별 상태를
관측하고 그 방향으로 활성화를 밀어 행동을 제어할 수 있음을 보였다[4,6]. 그러나 이들은
발견한 feature를 사람이 rollout 영상과 수동으로 정렬해 이름 붙일 뿐, 내부 표상이
사람이 정의한 행동 단계와 **어떤 해상도로** 대응하는지를 체계적으로 정량화하지
않는다. 내부 단계를 추론 중에 읽을 수 있으면 설명을 넘어 단계 조건부 개입[6]이나 실패
감지[7]의 조건 신호로도 쓸 수 있다. 본 연구는 GR00T N1.5(GR00T N1[1]의 후속 릴리스)가
RoboCasa[2] 주방 조작을 수행하는 동안의 action head 활성화를 라벨 없는 군집화만으로
분석하고, 모델의 내부 단계 표상이 사람이 붙인 단계 라벨(GT)과 맺는 관계를 시간 대조군
대비 margin으로 정량화한다.
