...
\rev{\rev{Detector의} 학습과 평가는 scene 단위로 분리한다. 10개 scene 중 8개 scene의 rollout 80개로 학습하고 나머지 2개 scene의 rollout 20개로 평가하며, 이 조합을 무작위로 3가지 만들어 조합마다 표준화, AE, clustering, \term{phase} 매핑을 학습 scene만으로 새로 적합한 뒤 3조합의 평균을 보고한다. AE와 KMeans 학습에는 사람이 주석한 \term{phase} 라벨(instruction당 3--6개)을 쓰지 않으며, 학습 scene의 라벨은 cluster를 최빈 \term{phase}에 대응시키는 매핑에만, 평가 scene의 라벨은 채점에만 사용한다.}
...
수와 같은 개수의 구간으로 \rev{등분위 분할해 얻은 상태열이다.} \rev{또한 외부 관찰만으로 \term{phase}를 나누는 기존 방법과 비교하기 위해, \textbf{행동 이벤트(behavioral event) 대조군}으로 기존 연구\cite{egsae}의 이벤트 발견 파이프라인을 채택한다. 저자가 공개한 구현을 수정 없이 사용하여,} 에피소드의 end-effector 궤적에서 AWE\cite{awe}로 waypoint를 추출하고, 각 waypoint 주변
...
\textbf{상호정보량}(MI) $I(Z;Y)=\sum_{k}\sum_{c} p(k,c)\log_2\frac{p(k,c)}{p(k)p(c)}$ 로 측정한다. 여기서 $p(k,c)$는 전체 스텝 중 \rev{cluster}가 $k$이면서 \term{phase}가 $c$인 비율이다. 다만 MI는 $k$가 커질수록
자동으로 커지므로 절대값만으로는 해상도가 다른 방법끼리 비교할 수 없다. \rev{그래서 앞의 Baseline에서 정의한, 같은 상태 수를 갖는 시간 대조군의 MI를 뺀 값을 판정 기준으로 사용한다:}
%
\begin{equation}
...
상태열 300개를 기준선으로 삼아 $z$-score로 측정한다.
