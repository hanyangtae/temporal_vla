...
\rev{\rev{Detector의} 학습과 평가는 scene 단위로 분리한다. 10개 scene 중 8개를 학습용, 나머지 2개를 평가용으로 나누어 각각 rollout 80개와 20개를 사용한다. 이러한 scene 분할을 무작위로 3가지 만들고, 분할마다 표준화, AE, clustering, \term{phase} 매핑을 학습 scene의 데이터만으로 새로 적합한다. 보고하는 수치는 세 분할 각각의 평가 scene에서 채점한 결과의 평균이다. AE와 KMeans 학습에는 사람이 주석한 \term{phase} 라벨(instruction당 3--6개)을 쓰지 않으며, 학습 scene의 라벨은 cluster를 최빈 \term{phase}에 대응시키는 매핑에만, 평가 scene의 라벨은 채점에만 사용한다.}
