...
\newcommand{\new}[1]{\textcolor{red}{#1}}
\newcommand{\term}[1]{#1}  % 용어 통일 감사 종료 (무색)
...
\begin{figure*}[!t]
  ...
  \caption{(a) Cluster$\times$GT \term{phase} contingency(PPCC candle, k=8). 최빈 \term{phase}로 행을 정렬하면 블록-대각 구조가 나타난다(순도 0.888). (b) scene 잔차화 후에도 \term{phase} margin은 유지되고, (c) scene MI는 급감한다.}
  \label{fig:purity}
\end{figure*}
\section{서 론}
...
\section{방 법}
\textbf{Data. } RoboCasa 주방 조작 환경에서 GR00T N1.5의 rollout을 수집한다. 대상은 5개 task(OpenDrawer, PnPCounterToCab, OvenRack, DishwasherRack, CoffeeSetupMug)에 속하는 10개 instruction이며, instruction마다 10개 scene에서 10회씩 총 100회의 rollout을 수집하였다. \rev{모델이 1회 추론 할 때 마다 action expert의 resudual stream activation을 캡쳐한다. 모델은 추론당 16개의 action step을 추론하며 그중 5개 action을 환경에서 실행하도록 세팅하였다. 이 activation은 12번째 layer의 마지막 denoising step에서 49개 토큰을 평균한 스텝당 1{,}536차원 벡터이고, GT \term{phase} 라벨도 같은 시점에서 읽는다.}
...
\chg{\textbf{Metric.} 판독 정확도와 별개로, cluster 라벨이 GT \term{phase}에 대해 담는 정보량은 \textbf{상호정보량}(MI)으로 측정한다. 다만 MI는 cluster 수가 늘면 자동으로 커지므로, 같은 상태 수를 갖는 시간 대조군의 MI를 뺀 \textbf{margin}을 판정 기준으로 사용한다. Margin이 양수라는 것은 ``에피소드의 몇 \% 지점인가''라는 시간 정보만으로는 설명되지 않는 \term{phase} 정보를 activation이 담고 있다는 뜻이다.}
...
\begin{figure}[!t]
  ...
  \caption{\chg{판독 정확도의 $k$ 강건성. 사람 \term{phase} 수(3--6, 회색 대역)를 넘는 $k\ge8$에서 성능이 평탄하다(0.82--0.88). 옅은 선은 instruction 10개 각각이다.}}
  \label{fig:ksweep}
\end{figure}
