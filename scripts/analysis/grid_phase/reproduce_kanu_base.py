...
PORTS = [8860, 8861]
GPU = 2
SESSION = 'codex-kanu-base-recovery-20260916'
...
def collect(row,port):
    ...
    with open(logpath,'ab') as log:
        proc=subprocess.Popen(['docker','exec','-e','MUJOCO_GL=egl','-e','CUDA_VISIBLE_DEVICES=2',
            '-e','MUJOCO_EGL_DEVICE_ID=2',
            '-e','__EGL_VENDOR_LIBRARY_FILENAMES='+cp(OUT/'egl_runtime/10_nvidia.json'),
            '-e','LD_LIBRARY_PATH='+cp(OUT/'egl_runtime')+':/usr/local/nvidia/lib:/usr/local/nvidia/lib64',
