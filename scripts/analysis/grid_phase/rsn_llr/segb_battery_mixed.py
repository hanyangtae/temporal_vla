...
# mixed-scene 필터: 성공·실패가 공존하는 scene 만 (전승/전패 scene 제외)
sc_cnt = {}
for e in ep_ids:
    s_, u = ep_meta[e]
    sc_cnt.setdefault(s_, [0, 0])[u] += 1
MIXED = {s_ for s_, v in sc_cnt.items() if v[0] >= 1 and v[1] >= 1}
ep_ids = np.array([e for e in ep_ids if ep_meta[e][0] in MIXED])
print(f"# mixed scenes {len(MIXED)}/{len(sc_cnt)}, ep {len(ep_ids)}")
