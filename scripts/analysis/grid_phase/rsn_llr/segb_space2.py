...
if AE:
    ...
    mu_g, std_g = ckpt["scaler"]["mu"], ckpt["scaler"]["scalar_std"]
    mu_g = np.asarray(mu_g, dtype=np.float32); std_g = float(std_g)
    Z = []
    with torch.no_grad():
        for i in range(0, len(X_raw), 8192):
            xb = torch.from_numpy((X_raw[i:i+8192] - mu_g) / std_g)
            Z.append(model.latent(xb).numpy())
    X_raw = np.concatenate(Z).astype(np.float32)
    print(f"# AE encode: {X_raw.shape}")
...
for pcode in sorted(set(pc.tolist())):
    ...
    for seed in range(5):
        ...
        V16 = Vt[:16]
        if AE:
            spaces = {"ae16": (lambda Z: Z, True)}
        else:
            spaces = {"raw": (lambda Z: Z, False),
    ...
    for sp_name in (["ae16"] if AE else ["raw", "pca16"]):
