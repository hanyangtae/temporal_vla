...
식별은 **좌표**(`<plan_id>/<machine>/<instruction>/s<i>/n<j>`)가 하고 `sig` 는 무결성
검증 열이 되었으므로(§3.1) 이 스크립트가 그 자리를 대신한다.
...
COORD_COLS = ("plan_id", "machine", "grid_instruction", "scene_idx", "noise_idx", "armsig")
...
class PlanInfo:
    ...
    def cell_keys(self) -> Iterator[tuple[str, int, int]]:
...
def parse_coords(rel: Path) -> tuple[str, str, str, int, int, str] | None:
    """grid-root 상대 meta 경로 → (plan_id, machine, instruction, s, n, arm).

    형태: <plan_id>/<machine>/<instruction...>/s<i>/n<j>/<arm>/<file>
    instruction 은 ``PPCC/bread`` 처럼 '/' 를 포함할 수 있어 machine 뒤부터 s<i> 앞까지
    전부다 (verify_grid.parse_coords 와 같은 규칙).
    """
    parts = rel.parts[:-1]          # 파일명 제거
    if len(parts) < 5:              # plan machine instr s n arm
        return None
    plan_id, machine, arm = parts[0], parts[1], parts[-1]
    m_s, m_n = _S_RE.match(parts[-3]), _N_RE.match(parts[-2])
    if not (m_s and m_n):
        return None
    instruction = "/".join(parts[2:-3])
    if not instruction:
        return None
    return plan_id, machine, instruction, int(m_s.group(1)), int(m_n.group(1)), arm
...
def build(grid_root: Path, plans: dict[str, PlanInfo]) -> tuple[list[dict], list[dict], list[str]]:
    ...
    for mp in sorted(grid_root.rglob("meta.json")):
        ...
        if coords is None:
            problems.append(f"{rel}: 좌표 경로가 아님 (<plan_id>/<machine>/<instr>/s/n/<arm>)")
            continue
        plan_id, machine, instruction, s_idx, n_idx, arm = coords
        try:
...
def report(grid_root: Path, plans: dict[str, PlanInfo], rollouts: list[dict],
    ...
    for plan_id in sorted(set(by_plan) | set(plans)):
        ...
        expected = Counter()
        if plan:
            for instr, s, n in plan.cell_keys():
                expected[instr] += 1
    ...
    # ── 중복 좌표 ──
    exact = Counter((r["plan_id"], r["machine"], r["grid_instruction"],
                     r["scene_idx"], r["noise_idx"]) for r in rollouts)
    dup_exact = [k for k, v in exact.items() if v > 1]
    cross: dict[tuple, set[str]] = defaultdict(set)
    for r in rollouts:
        cross[(r["plan_id"], r["grid_instruction"], r["scene_idx"], r["noise_idx"])].add(r["machine"])
    dup_cross = [k for k, v in cross.items() if len(v) > 1]
    print(f"\n## 중복 좌표")
    print(f"    같은 (plan,machine,instr,s,n) 에 base 2개  : {len(dup_exact)}")
