"""SlideDishwasherRack scene 실현가능성 사전 판정 (정책 무관, reset 시점 기하 스윕).

`drawer_scene_feasibility.py` 의 dishwasher 랙 이식판 (docs/steering/SCENE_FEASIBILITY.md §5).
템플릿에서 바꾼 건 네 가지다:

  ① fixture 참조 : `env.drawer` → `env.dishwasher`
  ② 관절        : `{fx.name}_slidejoint` → `fx._joint_names["rack"]`
                   (= `{naming_prefix}rack1_joint`). 정규화는 sign 을 손으로 다루지 않고
                   `Fixture.get_joint_state` / `set_joint_state` 의 규약을 그대로 쓴다
                   (`_joint_infos[j]["range"]`, joint_min<0 이면 1−t 반전).
  ③ 움직이는 body: 랙 관절 body 의 하위 트리 전부 (robocasa `check_rack_contact` 와 동일 경로)
  ④ 성공 임계    : 0.95 (`SlideDishwasherRack._check_success`: should_pull → rack ≥0.95,
                   라벨러 `SlidingRackPhaseLabeler.OPEN_TH` 와 동일)

★ 스윕 시작점이 drawer 와 다르다. out 에피소드는 `_setup_scene` 이
`dishwasher.slide_rack(self, value=0.5)` 로 랙을 **q=0.5 에서 시작**시킨다 (in 에피소드는
value=1.0). 따라서 스윕은 0 이 아니라 **현재값 → 1.0** 이다. 결과에 `q_start` 를 기록한다.
...
함정: 랙 **위**의 자유 물체(식기)는 상시 접촉이라 blocker 가 아니다. 랙과 함께 밀려 나오는
동역학이므로 기하 불가 판정 대상이 아님 → blocker 는 **자유관절(free joint) 조상을 갖지 않는
외부 geom** 만 인정한다 (정적 가구·벽·타 fixture).

부산물: seed 별 `ep_lang` (= `env.get_ep_meta()["lang"]`) 과 `should_pull` 을 같이 기록한다.
**SlideDishwasherRack 은 seed 마다 out/in variant 가 갈리므로** (kitchen_drawer.py
`should_pull = rng.random() > 0.5`, lang "Fully slide the top dishwasher rack {out|in}.")
이 필드가 나중에 out 필터와 feasibility 를 교차하는 근거다.
...
/temporal_vla/src/benchmarks/robocasa:/temporal_vla/src/benchmarks/robosuite:/temporal_vla" \
  python dishwasher_rack_feasibility.py --seeds 100000-100011 --out <path>
"""
...
def probe_one(env_name: str, seed: int, steps: int) -> dict:
    ...
    sim = k.sim
    try:  # instruction variant (out/in) — 정책 무관, reset 만의 함수
        ep_lang = str((k.get_ep_meta() or {}).get("lang", ""))
    except Exception:
        ep_lang = ""
    should_pull = bool(getattr(k, "should_pull", True))

    fx = k.dishwasher                        # ① fixture 참조
    pref = fx.naming_prefix
    jname = (getattr(fx, "_joint_names", {}) or {})["rack"]   # ② 관절
    ...
    def _is_external(gid: int) -> bool:
        ...
        if any(name.startswith(p) for p in _ROBOT_PREFIXES):
            return False
        # 랙 위 자유 물체(식기)는 blocker 가 아님 (랙과 함께 밀려 나옴)
...
def main() -> None:
    ...
    ap.add_argument("--task", default="SlideDishwasherRack")
    ...
    with ProcessPoolExecutor(max_workers=jobs) as ex:
        ...
        for s, fut in futs:
            ...
            print(f"[feas] seed {row['seed']}  q_start={row['q_start']:.3f}"
    ...
    if len(langs) > 1:  # out/in 처럼 seed 마다 instruction 이 갈리는 경우
