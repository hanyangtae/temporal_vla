#!/usr/bin/env bash
# Collect GR00T N1.5 RoboCasa rollouts for SAFE on a small 5-task set.
...
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/outputs/eval/robocasa/groot_n15/rollouts_${RUN_ID}}"
OUT_ROOT_CONTAINER="${OUT_ROOT_CONTAINER:-${CONTAINER_REPO_ROOT}/outputs/eval/robocasa/groot_n15/rollouts_${RUN_ID}}"
...
for task_id in "${!TASKS[@]}"; do
    ...
    for local_episode_idx in $(seq 0 $((EPISODES_PER_TASK - 1))); do
        ...
        if docker exec \
            ...
            "${ROBOCASA_CONTAINER}" \
            bash -lc "python '${CONTAINER_REPO_ROOT}/scripts/safe/groot_n15/robocasa/collect/collect_rollout.py' \
