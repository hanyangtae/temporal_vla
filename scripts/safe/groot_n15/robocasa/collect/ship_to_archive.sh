...
# v6 "instruction|s<sid>|j<jid>|n<nid>", legacy v5 "instruction|s<i>|k<r>|n<j>",
# legacy 2축 "instruction|s<i>|n<j>" (docs/04 §3.1.1).
set -uo pipefail
...
# 레이아웃(docs/04 §3.1.1):
#   v6        <plan_id>/<machine>/<instruction...>/s<sid>/j<jid>/n<nid>/<arm>
#   legacy v5 <plan_id>/<machine>/<instruction...>/s<i>/k<r>/n<j>/<arm>
#   legacy 2축 <plan_id>/<machine>/<instruction...>/s<i>/n<j>/<arm>
# 뒤에서부터 <arm>/<n..>/[<j..>|<k..>]/<s..> 를 읽어 지터 층 유무·종류를 자동 판별한다
# (지터 층은 ^j[0-9]+$ / ^k[0-9]+$ 일 때만 — instruction 층이 j·k 로 시작해도
#  s 층 위치로 구분된다).
cell_key_of() {
  local rel="$1"
  local IFS='/'
  ...
  local n=${#c[@]}
  [ "$n" -ge 6 ] || return 1
  local ni="${c[$((n - 2))]}" mid="${c[$((n - 3))]}"
  local si jit_layer instr_end
  if [[ "$mid" =~ ^[jk][0-9]+$ ]] && [ "$n" -ge 7 ]; then
    jit_layer="$mid"; si="${c[$((n - 4))]}"; instr_end=$((n - 4))
  else
    jit_layer=""; si="$mid"; instr_end=$((n - 3))
  fi
  local instr=""
  local i
