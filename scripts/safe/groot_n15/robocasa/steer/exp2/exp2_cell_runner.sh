#!/usr/bin/env bash
# exp2(구 pq2) 단일-arm cell 러너 (P2 선택 rollout + P3 held-out 공용).
# heldout_round_cell.sh 파생 — 차이: ①arm 1개/호출 ②EPLIST(비연속 episode 집합, P2 select-half)
# ③출력 루트 steer_eval_exp2/ (구 라운드 rename 충돌 회피) ④serve steer-preflight 로그를
...
SEROOT="steer_eval_exp2/${OUT_TIER}"
...
start_serves() {
  ...
  for i in $(seq 0 $((NW-1))); do
    docker exec -d -e CUDA_VISIBLE_DEVICES="${GPUS[$i]}" lerobot bash -lc \
      "cd /temporal_vla && setsid nohup python scripts/serve/lerobot.py --profile ${PROFILE} \
         ...
         --groot-dit-capture-layers ${CAP} ${flags} > /tmp/exp2_${CELL_ID}_${PORTS[$i]}.log 2>&1 < /dev/null &"
  done
  for port in "${PORTS[@]}"; do
    ...
    if docker exec lerobot bash -lc "grep -qiE 'Traceback|FAILED|FileNotFound' /tmp/exp2_${CELL_ID}_${port}.log"; then
      echo "[${CELL_ID}/${ARM_TAG}] ABORT ${port}"; docker exec lerobot bash -lc "tail -20 /tmp/exp2_${CELL_ID}_${port}.log"; exit 11
    fi
    # preflight 대조: steer arm 은 serve 가 로드한 npz·key·beta 가 스펙과 일치해야 rollout 시작
    if [ "$STEER_MODE" != base ]; then
      pf=$(docker exec lerobot bash -lc "grep '\[steer-preflight\]' /tmp/exp2_${CELL_ID}_${port}.log" || true)
