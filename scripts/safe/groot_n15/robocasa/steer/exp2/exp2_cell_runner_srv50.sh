#!/usr/bin/env bash
# exp2(구 pq2) 단일-arm cell 러너의 srv50(구 w2/worker2, A100) 변형 — serve 를 호스트 conda(lerobot_050_groot)로.
# exp2_cell_runner.sh 와 계약 동일 (ARM_SPEC/preflight/EPLIST/steer_eval_pq2), 차이는
# serve 기동·로그 경로(호스트)와 NPZ_DIR(호스트 절대경로) 뿐. srv50 에서 실행.
set -uo pipefail
...
SEROOT="steer_eval_pq2/${OUT_TIER}"  # srv50 원격 출력 — 원격은 구명(pq) 유지
...
start_serves() {
  ...
  for i in $(seq 0 $((NW-1))); do
    ( cd "$REPO_ROOT" && setsid nohup env CUDA_VISIBLE_DEVICES="${GPUS[$i]}" \
        ...
        --groot-dit-capture-layers ${CAP} ${flags} > "/tmp/exp2w_${CELL_ID}_${PORTS[$i]}.log" 2>&1 < /dev/null & )
  done
  local port ok pf
  for port in "${PORTS[@]}"; do
    ...
    if grep -qiE 'Traceback|FAILED|FileNotFound' "/tmp/exp2w_${CELL_ID}_${port}.log"; then
      echo "[${CELL_ID}/${ARM_TAG}] ABORT ${port}"; tail -20 "/tmp/exp2w_${CELL_ID}_${port}.log"; exit 11
    fi
    if [ "$STEER_MODE" != base ]; then
      pf=$(grep '\[steer-preflight\]' "/tmp/exp2w_${CELL_ID}_${port}.log" || true)
...
echo "[${CELL_ID}/${ARM_TAG}] $(date '+%F %T') srv50 mode=${STEER_MODE} eps=${#EPS[@]} tier=${OUT_TIER}"
...
{"cell":"${CELL_ID}","arm_tag":"${ARM_TAG}","mode":"${STEER_MODE}","npz_dir":"${NPZ_DIR:-}",
 ...
 "key":"${STEER_KEY}","eps":"${EPS[*]}","tier":"${OUT_TIER}","seed":"${SEED}","machine":"${MACHINE_TAG:-srv50-a100-gpu2}"}
EOF
start_serves
