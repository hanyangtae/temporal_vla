"""fit 표본 manifest 생성기 (재설계 라운드 exp2(구 pq2) — docs/steering/17 + Gate1 합의 구현).
...
def cmd_scene(args):
    ...
    # 원격(승준) 실행 기본값 — 원격은 구명(pq) 유지 (로컬은 symlink 호환)
...
def main():
    ...
    s.add_argument("--seed", default="pq2-20260710")  # 결정론 RNG seed 문자열 — 기존 manifest 재현 위해 구명 유지
    ...
    p.add_argument("--seed", default="pq2-20260710")  # 결정론 RNG seed 문자열 — 기존 manifest 재현 위해 구명 유지
