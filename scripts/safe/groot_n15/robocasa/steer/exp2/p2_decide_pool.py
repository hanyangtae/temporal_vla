...
def main():
    ...
    for sc in args.scenes:
        split = json.loads((G15 / "exp2_manifests" / sc / "split.json").read_text())
        eps = sorted(split["select_half"])[: args.k]
        base_s += sum(int(split["labels"][str(e)]) for e in eps)
        base_n += len(eps)
    root = G15 / "steer_eval_exp2/p2_pool" / args.scope
    rows = []
    ...
    out = G15 / "exp2_analysis" / f"selection_report_pool_{args.scope}.json"
