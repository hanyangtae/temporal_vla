...
# usage: SCENE=<scene> GPU=4 PA=8470 PB=8471 SINGLE_L=<layer> [FITN=30] bash p2_select_cell_v2.sh
set -uo pipefail
: "${SCENE:?}" "${GPU:?}" "${PA:?}" "${PB:?}"
...
MAN="outputs/eval/robocasa/groot_n15/exp2_manifests/$SCENE"
FITN="${FITN:-30}"
NPZ30="/temporal_vla/outputs/eval/robocasa/groot_n15/exp2_analysis/conceptors/$SCENE/per_scene_fit${FITN}/global"
...
run_one() { # tag layers beta
  ...
  bash scripts/safe/groot_n15/robocasa/steer/exp2/exp2_cell_runner.sh
}
