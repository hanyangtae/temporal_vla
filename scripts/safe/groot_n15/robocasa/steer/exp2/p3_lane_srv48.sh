#!/bin/bash
# P3 lane runner (.48 오케스트레이션): 로컬에서 실행, arm 을 .48(srv48(구 w3/worker1) A100 GPU2)에 ssh 로 위임.
# p3_lane_srv50g0.sh 의 .48 변형 — 차이: 호스트, GPU2, HF_HOME 주입(체크포인트가 전용 캐시
# ~/.cache/temporal_vla/datasets/huggingface 에 있음 — HF hub 캐시는 타인 자산이라 미사용),
# HF_HUB_OFFLINE=1(네트워크 resolve 차단), machine 태그.
# usage: p3_lane_srv48.sh <LANE> <P1> <P2> <P3port>
set -u
...
SRV48="junhyeong@166.104.35.48"  # srv48(구 w48)
HREPO=/home/junhyeong/pkt_ws/temporal_vla
MACHINE=srv48-a100-gpu2
...
echo "[$LANE] p3 srv48-gpu2 start ports=$W_P1,$W_P2,$W_P3 $(date -u '+%FT%T')"
