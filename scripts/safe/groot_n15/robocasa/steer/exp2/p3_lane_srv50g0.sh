#!/bin/bash
# P3 lane runner (srv50 오케스트레이션): 로컬에서 실행, arm 을 srv50 에 ssh 로 위임.
# 산출 pkl 은 srv50 보관, 로컬은 파일명 manifest(판정 출처)+MACHINE 만 — v2 직송 패턴의 경량판.
# usage: p3_lane_srv50g0.sh <LANE> <P1> <P2> <P3port>
set -u
...
echo "[$LANE] p3 srv50-gpu0 start ports=$W_P1,$W_P2,$W_P3 $(date -u '+%FT%T')"
...
finish() { ( flock 9
    printf '%s\t%s\t%s\t%s\n' "$(date -u '+%FT%T')" "srv50-a100-gpu2" "$1" "$2" >> "$Q/ledger.tsv"
    rm -f "$Q/running/${LANE}.job" ) 9>>"$Q/lock"; }

while true; do
  free_kb=$($SSH "df -k / | tail -1 | awk '{print \$4}'" 2>/dev/null || echo 0)
  [ "${free_kb:-0}" -lt 10485760 ] && { echo "[$LANE] srv50 디스크/ssh 문제 — 10분 대기"; sleep 600; continue; }
...
ARM_TAG=${F[TAG]} STEER_MODE=${F[MODE]} EP0=60 EP1=119 OUT_TIER=p3 GPUS_L='0 0 0' PORTS_L='$W_P1 $W_P2 $W_P3' $STEER_ENV \
bash scripts/safe/groot_n15/robocasa/steer/exp2/exp2_cell_runner_srv50.sh"
  $SSH "$RCMD" >> "$Q/logs_${LANE}.log" 2>&1
  RRD="outputs/eval/robocasa/groot_n15/steer_eval_pq2/p3/$c/${F[TAG]}/raw_rollouts/$task/$c"  # srv50 원격 — 원격은 구명(pq) 유지
  ...
  printf '%s machine=srv50-a100-gpu2 eps=%s\n' "$(date -u '+%FT%T')" "$n" >> "$LD/MACHINE.txt"
