#!/usr/bin/env python3
"""exp3(구 pq3) eval 집계·판정 (계획서 v9 §F) — EXPECT_N=30, seen/unseen 분해, Holm 6 primary.

판정 규칙은 exp3_decision.py (구 pq3_decision.py — 동결 모듈 — sha256 을 summary 에 기록) 단일 출처.
...
import exp3_decision as D  # noqa: E402
from make_exp3_manifests import STEM_RE  # noqa: E402
...
def main() -> None:
    ...
    ap.add_argument("--eval-root", required=True, help="steer_eval_exp3/e1")
    ap.add_argument("--manifest-dir", required=True)
    ap.add_argument("--arm-config", required=True, help="build_exp3_queue 와 동일 json (cells)")
    ...
    ap.add_argument("--gated-gate-report", required=True,
                    help="exp3_gated_gate.py --enforce 산출 json — gated N/A 를 여기서 "
    ...
    decision_sha = hashlib.sha256(
        (Path(__file__).parent / "exp3_decision.py").read_bytes()
    ).hexdigest()[:16]
    ...
    md = ["# exp3 집계 (판정=pooled McNemar+Holm 단독, seen/unseen 은 방향 읽기용)", ""]
    ...
    print(f"[aggregate-exp3] -> {out_dir} (status={status['overall']}, "
