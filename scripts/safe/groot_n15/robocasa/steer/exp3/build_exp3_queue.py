#!/usr/bin/env python3
"""exp3(구 pq3) eval 큐 빌더 (계획서 v9 §E) — 4 arm × 5 cell × 30판, host 균형 배정.
...
입력 --arm-config (Gate D 산출 단일 출처, json):
{
  ...
  "hosts": ["local", "local", "local", "srv50", "srv48"]     # lane 클래스 (반복 = 가중; 구 w2/w48 도 읽기 호환)
}
...
gated 성립 게이트: --gated-gate-report(exp3_gated_gate.py 산출 json)의 task 별 pass 가
...
def main() -> None:
    ...
    ap.add_argument("--gated-gate-report", default=None,
                    help="exp3_gated_gate.py 산출 json ({task: {pass: bool}}) — gated arm "
    ...
    if has_gated:
        ...
        if not_enforced:
            raise SystemExit(
                ...
                "판정용은 exp3_gated_gate.py --enforce 산출만 허용"
            )
    ...
    print(f"[build-exp3-queue] {len(rows)} rows -> {qroot / 'queue.tsv'} "
