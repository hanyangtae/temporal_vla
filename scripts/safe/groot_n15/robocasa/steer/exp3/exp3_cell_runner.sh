#!/usr/bin/env bash
# exp3(구 pq3) 단일-arm cell 러너 (exp2_cell_runner.sh 파생 — scene-diverse eval 전용).
...
SEROOT="steer_eval_exp3/${OUT_TIER}"
...
start_serves() {
  ...
  sleep 3
  for i in $(seq 0 $((NW-1))); do
    docker exec lerobot bash -lc "rm -f /tmp/exp3_${CELL_ID}_${PORTS[$i]}.log"
    # 캡처 플래그 없음 (eval activation 미저장) — exp2 러너와의 핵심 차이
    docker exec -d -e CUDA_VISIBLE_DEVICES="${GPUS[$i]}" lerobot bash -lc \
      "cd /temporal_vla && setsid nohup python scripts/serve/lerobot.py --profile ${PROFILE} \
         ...
         > /tmp/exp3_${CELL_ID}_${PORTS[$i]}.log 2>&1 < /dev/null &"
  done
  for port in "${PORTS[@]}"; do
    ...
    LOG="/tmp/exp3_${CELL_ID}_${port}.log"
