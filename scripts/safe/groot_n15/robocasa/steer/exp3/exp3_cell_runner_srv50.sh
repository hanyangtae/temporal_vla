#!/usr/bin/env bash
# exp3(구 pq3) 단일-arm cell 러너의 원격 워커 변형 (srv50(구 w2/worker2)/.50 기본, srv48(구 w48) 은 MACHINE_TAG·HF_HOME_OVERRIDE
# env 로 동일 사용) — serve 를 호스트 conda 로. 계약·preflight 는 exp3_cell_runner.sh 와 동일
# (boot_id·health 지문·ARM_SPEC 불변·run_tag 포함), serve 기동·로그·health 만 호스트 측.
#
# env: exp3_cell_runner.sh 와 동일 + [HPY(serve python)] [HF_HOME_OVERRIDE] [MACHINE_TAG]
set -uo pipefail
...
SEROOT="steer_eval_pq3/${OUT_TIER}"  # 워커 원격 출력 — 원격은 구명(pq) 유지
...
start_serves() {
  ...
  for i in $(seq 0 $((NW-1))); do
    rm -f "/tmp/exp3w_${CELL_ID}_${PORTS[$i]}.log"
    ( cd "$REPO_ROOT" && setsid nohup env CUDA_VISIBLE_DEVICES="${GPUS[$i]}" \
        ...
        > "/tmp/exp3w_${CELL_ID}_${PORTS[$i]}.log" 2>&1 < /dev/null & )
  done
  ...
  for port in "${PORTS[@]}"; do
    ...
    LOG="/tmp/exp3w_${CELL_ID}_${port}.log"
...
echo "[${CELL_ID}/${ARM_TAG}] $(date '+%F %T') ${MACHINE_TAG:-srv50} mode=${STEER_MODE} eps=${#EPS[@]}/${EXPECT_N} tier=${OUT_TIER} manifest=${MANIFEST_SHA}"
mkdir -p "${OUT_HOST}/${ARM_TAG}"
...
printf '{"spec_sha":"%s","machine":"%s","manifest":"%s","core":%s}\n' \
  "$SPEC_SHA" "${MACHINE_TAG:-srv50-a100}" "$MANIFEST" "$CORE_SPEC" > "${OUT_HOST}/${ARM_TAG}/ARM_SPEC.json"
start_serves
PIDS=()
