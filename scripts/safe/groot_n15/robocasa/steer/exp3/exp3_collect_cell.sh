#!/usr/bin/env bash
# exp3(구 pq3) fit 수집 러너 — full-token capture ON, scene-diverse (episode i → seed_i).
# exp2_cell_runner.sh 의 수집 계열 파생. 계획서 v9 §C + 파이프라인 규약:
...
FITROOT="outputs/eval/robocasa/groot_n15/phase_event_exp3/raw_rollouts"
...
# 승준 원격 — 원격은 구명(pq) 유지 (로컬 FITROOT rename 과 무관하게 기존 원격 dir 로 직송)
SJ_ROOT="${SJ_ROOT:-~/workspace/temporal_vla/outputs/eval/robocasa/groot_n15/phase_event_pq3/raw_rollouts}"
...
start_serves() {
  ...
  for i in $(seq 0 $((NW-1))); do
    docker exec lerobot bash -lc "rm -f /tmp/exp3c_${CELL_ID}_${PORTS[$i]}.log"
    docker exec -d -e CUDA_VISIBLE_DEVICES="${GPUS[$i]}" lerobot bash -lc \
      "cd /temporal_vla && setsid nohup python scripts/serve/lerobot.py --profile ${PROFILE} \
         ...
         > /tmp/exp3c_${CELL_ID}_${PORTS[$i]}.log 2>&1 < /dev/null &"
  done
  for port in "${PORTS[@]}"; do
    ...
    LOG="/tmp/exp3c_${CELL_ID}_${port}.log"
