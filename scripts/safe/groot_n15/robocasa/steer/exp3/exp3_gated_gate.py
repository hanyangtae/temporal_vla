#!/usr/bin/env python3
"""exp3(구 pq3) gated 성립 게이트 (계획서 v9 §arm gated fit 게이트, Codex R2#2 / Gate2 높음#6).
...
을 산출해 task 별 pass 판정 json 을 쓴다. build_exp3_queue 는 이 report 의 pass 없이
...
def load_manifest_rolls(manifest: Path, token_pool: str):
    ...
    for line in manifest.read_text().splitlines():
        ...
        if d.get("capture_token_mode") != FULLTOKEN_MODE:
            raise SystemExit(f"{pkl}: full-token pkl 아님 (gated 게이트는 exp3 수집분 전용)")
