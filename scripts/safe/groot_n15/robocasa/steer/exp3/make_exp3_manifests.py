#!/usr/bin/env python3
"""exp3(구 pq3) seed 레이아웃 manifest 생성기 (계획서 dynamic-riding-aurora v9 §Seed 레이아웃).

exp2 make_fit_manifests.py 는 디스크의 전 episode 를 층화 split 하므로 exp3 의
scene-diverse seed 구획(fit S1-S15+backfill / eval 30 예약·동결 / sweep=fit 재사용)을
강제할 수 없다 (Codex Gate1 R1#1). 이 도구가 exp3 의 seed 단일 출처다.
