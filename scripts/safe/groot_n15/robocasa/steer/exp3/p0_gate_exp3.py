#!/usr/bin/env python3
"""exp3(구 pq3) fit 수집 게이트 (계획서 v9 §C) — exp2 p0_gate.py 파생, 경로 인자화.

게이트: fit 목표 판수 수집 완료 ∧ succ≥3 ∧ fail≥3 (env 원판정 — exp3 에 stove 없음,
...
from make_exp3_manifests import _read_plan, scan_collected  # noqa: E402
...
def main() -> None:
    ...
    ap.add_argument("--manifest-dir", required=True, help="make_exp3_manifests --out-dir/<cell>")
    ...
    if outside:
        print(f"[p0-exp3] 수집 ep 가 plan 밖: {outside} (seed 구획 위반)", file=sys.stderr)
        sys.exit(4)
    ...
    if eps_sorted and eps_sorted != list(range(len(eps_sorted))):
        ...
        print(f"[p0-exp3] 수집 ep 가 plan prefix 가 아님: {eps_sorted}", file=sys.stderr)
        sys.exit(4)
    ...
    if need_more > 0 or not class_ok:
        ...
        if not backfill:
            print(f"[p0-exp3] plan 소진 (S{plan_idx[-1] + 1} 상한) — cell 탈락", file=sys.stderr)
            sys.exit(3)
        print("BACKFILL_EPLIST=" + " ".join(str(i) for i in backfill))
        sys.exit(2)

    print(json.dumps(report, ensure_ascii=False))
    print(f"[p0-exp3] GATE PASS n={n} succ={succ} fail={fail}")
