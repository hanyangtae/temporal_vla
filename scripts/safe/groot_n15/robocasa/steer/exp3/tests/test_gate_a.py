#!/usr/bin/env python3
"""exp3(구 pq3) Gate A 테스트 (계획서 v9 §A Gate A — 배선 사고 방지 유닛/parity/회귀).

lerobot 컨테이너(CPU)에서 실행 — pytest 없이도 동작:
  docker compose run --rm lerobot python /temporal_vla/scripts/safe/groot_n15/robocasa/steer/exp3/tests/test_gate_a.py
...
def test_7_pkl_gate_and_per_step_npz(tmp: Path):
    ...
    # exp3 fit 은 --eval-reserved 필수 + manifest cell 집합과 reserved cell_id 일치 강제
    ...
    # reserved 누락 시 exp3 fit 은 exit 5 (R3 높음#2 회귀)
    r = subprocess.run(
        [sys.executable, str(fit), "--cell", "x/pq3_test", "--manifest", str(man_new),
         "--groups", "global", "--require-capture-token-mode", "all_token_full",
...
def test_8_seed_separation_regression(tmp: Path):
    mk = EXP3 / "make_exp3_manifests.py"
