...
# layer = conceptor_layer_sweep.json 의 selected_layer (분산 기준 sweep — 전 layer 퇴화 확정이어도
...
# 출력을 serve 계약(<cell>/conceptor/steer/dit_L{L}/conceptors.npz)으로 복사 + provenance 기록
...
for c in pq3_ppcc_bread:PickPlaceCounterToCabinet pq3_ppcc_beer:PickPlaceCounterToCabinet \
  ...
  echo "=== [conceptor fit] $cell layer=L$lyr $(date -u '+%FT%T') ==="
  ...
  if [ ! -f "$NPZ" ]; then
    echo "[conceptor fit] $cell floor_exhausted → quota-floor 0 폴백 (퇴화 배포, legacy 참조선)"
    fit_once 0
  fi
  if [ -f "$NPZ" ]; then
    D="$O/$cell/conceptor/steer/dit_L$lyr"; mkdir -p "$D"
...
echo "=== [conceptor fit] all done rc=$rc $(date -u '+%FT%T') ==="
exit $rc
