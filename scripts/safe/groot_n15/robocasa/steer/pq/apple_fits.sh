#!/bin/bash
# A-S2: apple per-seed(ps15/30/60 ×4) + xa pool + gx(8-cell) pool fit → NPZ worker2 push → NPZ_READY.
...
rsync -aR outputs/eval/robocasa/groot_n15/phase_event_6p/analysis/final_ps15 \
  ...
  "$W2:pkt_ws/temporal_vla/" || { echo "[A-S2] NPZ push 실패"; exit 3; }
