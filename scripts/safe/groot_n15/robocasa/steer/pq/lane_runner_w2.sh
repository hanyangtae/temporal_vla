#!/bin/bash
# worker2 lane runner (로컬에서 실행): 큐 pop → ssh로 worker2에서 heldout_round_cell_host.sh 실행
# → 완료 arm rsync pull → 검증 → worker2 원본 pkl/mp4 삭제 → MACHINE.txt 기록.
# usage: lane_runner_w2.sh <lane_id> <p1> <p2> <p3>   (GPU2 고정, 3-worker)
set -u
...
cd "$REPO"
echo "[$LANE] start w2 GPU2 ports=$P1,$P2,$P3 $(date -u '+%FT%T')"

while true; do
  # 디스크 가드 (worker2 여유 <10GB → 대기)
...
${layers:+STEER_LAYERS=$layers} bash scripts/safe/groot_n15/robocasa/steer/heldout_round_cell_host.sh"
  ...
  mkdir -p "$LDIR"
  rsync -a --min-size=1 "$W2:$RDIR" "$LDIR" 2>/dev/null
  pull_rc=$?
  after=$(pkl_count "$cell" "$tag")
  mark_machine "$REPO/$SE/$c/$tag" "worker2-a100-gpu2" "$((after - before))"
  if [ "$after" -ge 60 ] && [ "$pull_rc" -eq 0 ]; then
    ...
    finish_job "$line" "$LANE" "worker2-a100-gpu2" "$((after - before))" "$after"
  else
