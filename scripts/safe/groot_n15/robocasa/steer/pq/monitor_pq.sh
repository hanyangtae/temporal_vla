...
# 0=전작업 완료 / 2=러너 전멸(큐 남음) / 3=로컬 디스크 <10GB / 4=worker2 접속 불가 지속 / 5=failed 발생
set -u
...
while true; do
  ...
  if ! ssh -o BatchMode=yes -o ConnectTimeout=10 "$W2" true 2>/dev/null; then
    w2fail=$((w2fail+1)); [ $w2fail -ge 3 ] && { echo "EVENT: worker2 ssh 3연속 실패"; exit 4; }
  else w2fail=0; fi
  sleep 600
done
