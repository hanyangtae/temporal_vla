...
큐 행 = pq2_cell_runner 가 그대로 소비할 env 조각 (NPZ 는 repo 상대경로 — 러너가 자기
머신 경로로 확장). 형식: KEY=VAL 공백연결 한 줄 + '|retry' 접미.

usage: build_p3_queue.py --out <queue.tsv> [--selection-dir pq2_analysis]
...
CONC = "outputs/eval/robocasa/groot_n15/pq2_analysis/conceptors"
...
def main():
    ...
    ap.add_argument("--selection-dir", default=str(G15 / "pq2_analysis"))
    args = ap.parse_args()
