...
def arm_sr(scene: str, tag: str, task: str):
    d = G15 / "steer_eval_pq2/p2" / scene / tag / "raw_rollouts" / task / scene
...
def main():
    ...
    args = ap.parse_args()
    split = json.loads((G15 / "pq2_manifests" / args.scene / "split.json").read_text())
    ...
    arms = args.arms or sorted(p.name for p in (G15 / "steer_eval_pq2/p2" / args.scene).glob("p2_*"))
    rows = []
    for tag in arms:
    ...
    out = G15 / "pq2_analysis" / f"selection_report_{args.scene}.json"
