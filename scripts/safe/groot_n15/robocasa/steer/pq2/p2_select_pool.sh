...
cd "$MYREPO"
source scripts/safe/groot_n15/robocasa/steer/pq/pq_lib.sh  # REPO 덮임 주의 — MYREPO 만 사용
PROFILE=configs/checkpoints/lerobot_groot_n15__robocasa365_ckpt120000.yaml
CAP="0,2,4,8,10,12,15"
NPZ_DIR="/temporal_vla/outputs/eval/robocasa/groot_n15/pq2_analysis/conceptors/_pools/${NPZ_SUB}/global"
OUT_HOST="$MYREPO/outputs/eval/robocasa/groot_n15/steer_eval_pq2/p2_pool/${SCOPE}"
OUT_CONT="/temporal_vla/outputs/eval/robocasa/groot_n15/steer_eval_pq2/p2_pool/${SCOPE}"
PYPATH="/temporal_vla/src/policies/Isaac-GR00T:/temporal_vla/src/benchmarks/robocasa:/temporal_vla"
mkdir -p "$OUT_HOST/logs"

eps_of() { python3 -c "import json; print(' '.join(map(str, sorted(json.load(open('outputs/eval/robocasa/groot_n15/pq2_manifests/$1/split.json'))['select_half'])[:$K])))"; }
...
serve_up() { docker exec -d -e CUDA_VISIBLE_DEVICES="$GPU" lerobot bash -lc \
  "cd /temporal_vla && setsid nohup python scripts/serve/lerobot.py --profile ${PROFILE} \
     ...
     > /tmp/pq2pool_${SCOPE}_${1}.log 2>&1 < /dev/null &"; }
...
run_arm() { # tag layers beta
  ...
  local pf
  pf=$(docker exec lerobot bash -lc "grep '\[steer-preflight\]' /tmp/pq2pool_${SCOPE}_${PA}.log" || true)
