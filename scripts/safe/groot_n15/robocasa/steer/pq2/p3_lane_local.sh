#!/bin/bash
# P3 lane runner (로컬 GPU): work_queue_p3 에서 arm pop → pq2_cell_runner.sh.
# usage: p3_lane_local.sh <LANE> <GPU> <PA> <PB>
set -u
LANE=$1 GPU=$2 PA=$3 PB=$4
MYREPO=$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../../.." && pwd)
cd "$MYREPO"
source scripts/safe/groot_n15/robocasa/steer/pq/pq_lib.sh  # CELLS/row_of 만 사용 (REPO 덮임 — MYREPO 신뢰)
Q=$MYREPO/outputs/eval/robocasa/groot_n15/work_queue_p3
RUNNER=scripts/safe/groot_n15/robocasa/steer/pq2/pq2_cell_runner.sh
...
while true; do
  ...
  d="$MYREPO/outputs/eval/robocasa/groot_n15/steer_eval_pq2/p3/$c/${F[TAG]}/raw_rollouts/$task/$c"
