...
echo "[$LANE] p3 w3(worker1) start ports=$W_P1,$W_P2 $(date -u '+%FT%T')"
...
finish() { ( flock 9
    printf '%s\t%s\t%s\t%s\n' "$(date -u '+%FT%T')" "worker1-a100-gpu2" "$1" "$2" >> "$Q/ledger.tsv"
    rm -f "$Q/running/${LANE}.job" ) 9>>"$Q/lock"; }
...
bash scripts/safe/groot_n15/robocasa/steer/pq2/pq2_cell_runner_w2.sh"
  ...
  printf '%s machine=worker1-a100-gpu2 eps=%s\n' "$(date -u '+%FT%T')" "$n" >> "$LD/MACHINE.txt"
