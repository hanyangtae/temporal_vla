...
#   PERM_NPZ PERM_LAYERS GATED_NPZ GATED_LAYERS (Stage1 layer 확정본)
...
for arm in $ARMS; do
  case "$arm" in
    perm)  NPZ="${PERM_NPZ:?}";  LAYERS="${PERM_LAYERS:?}" ;;
    gated) NPZ="${GATED_NPZ:?}"; LAYERS="${GATED_LAYERS:?}" ;;
  ...
  for beta in $BETAS; do
    ...
    NPZ_DIR="$NPZ" STEER_LAYERS="$LAYERS" STEER_BETA="$beta" \
