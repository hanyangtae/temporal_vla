...
OUT_ROOT="${OUT_ROOT:-outputs/eval/robocasa/groot_n15/steer_eval_pq3/c0_scan}"
...
NEW_CFG="$REPO_ROOT/configs/robocasa/pq3_ppcc_new_cells.tsv"
...
OUT_TSV="$OUT_ROOT/pq3_ppcc_new_seeds.tsv"
for CELL in $(awk -F'\t' 'NR>1{print $2}' "$NEW_CFG"); do
  docker exec -e MUJOCO_GL=egl -e PYTHONPATH="$PYPATH" robocasa \
    python "$SCANNER" \
    --cells-config "/temporal_vla/configs/robocasa/pq3_ppcc_new_cells.tsv" \
    --cell-id "$CELL" \
...
echo "[c0] 완료 -> $REPO_ROOT/$OUT_TSV (make_pq3_manifests plan 의 --seeds-tsv 로 사용)"
