#!/usr/bin/env bash
# pq3 fit 수집 러너 — 원격 워커(w2/.50·w48) 변형. pq3_collect_cell.sh 와 계약 동일하되
# serve 를 호스트 conda 로 기동(pq3_cell_runner_w2.sh 방식). SHIP 은 워커→승준 직송
...
SJ_ROOT="${SJ_ROOT:-~/datasets/temporal_vla_outputs/eval/robocasa/groot_n15/phase_event_pq3/raw_rollouts}"
...
for ep in "${EPS[@]}"; do
  [ -n "${ENVSEED[$ep]:-}" ] || { echo "[collect-w2 ${CELL_ID}] ep=${ep} 가 collect_plan 에 없음 ABORT"; exit 13; }
done
...
start_serves() {
  ...
  for i in $(seq 0 $((NW-1))); do
    local LOG="/tmp/pq3c_${CELL_ID}_${PORTS[$i]}.log"; rm -f "$LOG"
  ...
  for port in "${PORTS[@]}"; do
    local ok=0 LOG="/tmp/pq3c_${CELL_ID}_${port}.log"
    for _ in $(seq 1 180); do
      curl -s -m 3 "http://127.0.0.1:${port}/health" 2>/dev/null | grep -q '"status":"ok"' && { ok=1; break; }
      sleep 5
    done
    [ $ok = 1 ] || { echo "[collect-w2 ${CELL_ID}] serve ${port} TIMEOUT"; exit 11; }
    grep -qiE 'Traceback|FAILED|FileNotFound|Address already in use|Errno 98' "$LOG" \
      && { echo "[collect-w2 ${CELL_ID}] serve ABORT ${port}"; tail -20 "$LOG"; exit 11; }
    ...
    [ -n "$boot_log" ] && [ "$boot_log" = "$boot_http" ] \
      || { echo "[collect-w2 ${CELL_ID}] boot_id 불일치 ABORT"; exit 12; }
    echo "$health" | grep -q '"capture_token_mode":"all_token_full"' \
      || { echo "[collect-w2 ${CELL_ID}] capture_token_mode != all_token_full ABORT"; exit 12; }
  done
}

ship_ep() { # stem — 승준 직송 3중 검증(pq3_collect_cell.sh 동일 표준)
...
run_w() {
  ...
  for ep in "${EPS[@]}"; do
    ...
    if ! ls "${stem_glob}"*.csv >/dev/null 2>&1; then
      ...
      if [ "$rc" -ne 0 ]; then echo "[collect-w2 ${CELL_ID}] ep${ep} rc=${rc}"; fail=1; continue; fi
    fi
    if [ "$SHIP" = 1 ]; then
      for st in "${stem_glob}"1 "${stem_glob}"0; do
        [ -e "${st}.pkl" ] && { ship_ep "$(basename "$st" .pkl)" || { echo "[collect-w2 ${CELL_ID}] ep${ep} 직송 실패"; fail=1; }; }
      done
    fi
  done
  return $fail
...
echo "[collect-w2 ${CELL_ID}] $(date '+%F %T') ${MACHINE_TAG:-w2} eps=${#EPS[@]} ship=${SHIP} manifest=$(sha256sum "$MANIFEST" | cut -c1-12)"
start_serves
PIDS=()
for wid in $(seq 0 $((NW-1))); do
  run_w "$wid" "${PORTS[$wid]}" > "${LOGDIR}/collect_w2_w${wid}.log" 2>&1 & PIDS+=($!)
done
WFAIL=0
...
for ep in "${EPS[@]}"; do
  ...
  [ "$cnt" -eq 1 ] || { echo "[collect-w2 ${CELL_ID}] ep${ep} csv 스템 ${cnt}개 (기대 1)"; MISS=1; continue; }
  if [ "$SHIP" = 1 ]; then
    stem=$(basename "$(ls "$d"/task${CELL_INDEX}--ep${ep}--succ*.csv)" .csv)
    grep -q "	${stem}.pkl	" "$d/SHIPPED.tsv" 2>/dev/null \
      || { echo "[collect-w2 ${CELL_ID}] ep${ep} SHIPPED 미등재"; MISS=1; }
    [ ! -e "$d/${stem}.pkl" ] || { echo "[collect-w2 ${CELL_ID}] ep${ep} 로컬 pkl 잔존"; MISS=1; }
  fi
done
[ "$WFAIL" = 0 ] && [ "$MISS" = 0 ] \
  && echo "[collect-w2 ${CELL_ID}] $(date '+%F %T') DONE stems=$(ls "$d"/task${CELL_INDEX}--ep*--succ*.csv 2>/dev/null | wc -l) (기대 ${#EPS[@]}, wfail=${WFAIL} miss=${MISS})" \
  || { echo "[collect-w2 ${CELL_ID}] FAIL wfail=${WFAIL} miss=${MISS}"; exit 1; }
