#!/bin/bash
# srv50(구 w2/worker2) lane runner v2: 산출 pkl/mp4를 로컬에 안 쌓고 승준으로 직송(로컬은 파이프만).
#  - 로컬 수신물 = 경량 메타(파일명 manifest, sr tsv, MACHINE.txt, ledger)뿐
#  - 무거운 것(pkl/mp4) = ssh srv50 tar | ssh 승준 tar 스트림 → 승준 검증 → srv50 삭제
# usage: lane_runner_srv50_v2.sh <lane_id> <p1> <p2> <p3>
set -u
...
cd "$REPO"
echo "[$LANE] start srv50-v2 GPU2 ports=$P1,$P2,$P3 $(date -u '+%FT%T')"
...
while true; do
  ...
  if [ "$jt" = collect ]; then
    # P0 fit 재료 수집 (ep0-59) → 승준 직송 (fit 은 승준에서 — exp2(구 pq2) 재설계)
    # srv50 원격 repo 는 git sync 로 신명(exp2) 경로를 따라감 — 실행 전 srv50 checkout 최신화 필요
    RCMD="cd $HREPO && CELL_ID=$c TASK=$task ENVN=$env CELL_INDEX=$ci SEED=$seed INSTR=$(printf '%q' "$instr") \
GPUS_L='2 2 2' PORTS_L='$P1 $P2 $P3' bash scripts/safe/groot_n15/robocasa/steer/exp2/srv50_collect_cell.sh"
    ...
    mark_machine "$LFIT" "srv50-a100-gpu2(direct-sj)" "$n_pkl"
    if [ "$n_pkl" -ge 60 ]; then
      $SSH "cd $HREPO && tar cf - $FRD" | ssh -o BatchMode=yes -p $SJP $SJ "cd $SJREPO && tar xf -" \
        2>> "$QROOT/logs/srv50_${LANE}.log"
      ship_rc=$?
      ...
      if [ "$ship_rc" -eq 0 ] && [ "${sj_n:-0}" -ge "$n_pkl" ]; then
        ...
        finish_job "$line" "$LANE" "srv50-a100-gpu2(sj=$sj_n)" "$n_pkl" "$n_pkl"
      else
        echo "[$LANE] $c collect 직송 검증 실패(ship_rc=$ship_rc sj=$sj_n/$n_pkl) — srv50 원본 보존, 재큐"
...
${layers:+STEER_LAYERS=$layers} bash scripts/safe/groot_n15/robocasa/steer/heldout_round_cell_host.sh"
  ...
  mark_machine "$LTAG" "srv50-a100-gpu2(direct-sj)" "$n_pkl"
  ...
  if [ "$n_pkl" -ge 60 ]; then
    ...
    if [ "$ship_rc" -eq 0 ] && [ "${sj_n:-0}" -ge "$n_pkl" ]; then
      ...
      echo "[$LANE] $tag 직송 검증 실패(ship_rc=$ship_rc sj=$sj_n/$n_pkl) — srv50 원본 보존, 재큐"
      requeue_job "$line" "$LANE"
    fi
  else
