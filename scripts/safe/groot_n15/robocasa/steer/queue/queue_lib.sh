...
SRV50=junhyeong@166.104.35.50  # srv50(구 w2/worker2)
SRV50REPO='~/pkt_ws/temporal_vla'
...
CELLS=(
  ...
  # --- exp2(구 pq2) P0 apple 신규 scene 후보 (2026-07-10, seed 순 — selected_instruction_seeds.tsv 중 미사용 상위 6) ---
  ...
  # --- exp3(구 pq3) cells (2026-07-15, cell=instruction — scene 은 manifest 의 seed 로 다양화;
  #     SEED 열은 대표값일 뿐 exp3 러너는 make_exp3_manifests 산출 manifest 를 사용;
  #     cell id 의 pq3_ 접두는 on-disk 데이터 키라 유지) ---
  ...
  # exp3 ppcc 신규 2종 (C0 스캔 2026-07-16 확정: 빈도 beer 9·pizza cutter 8 / 400, SR 미사용;
  # configs/robocasa/exp3_ppcc_new_cells.tsv canonical 과 일치, SEED 열은 대표값)
...
# pop_job <class:local|srv50|srv48> <lane_id> → stdout에 행, 없으면 rc=1
# (class 값은 "local" 만 특별취급 — local 이 아니면 어떤 라벨이든 non-local lane 으로 동작해
#  하위호환 필요 없음: 기존 w2/w48 호출자도 그대로 동작)
pop_job() {
