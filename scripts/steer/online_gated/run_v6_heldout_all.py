...
def check_sidecars(out, arm, cell, rows):
    slug,s,j = cell
    folder = out/arm/f'{slug}_s{s}_j{j}'/slug/ARMS[arm][0]/'raw_rollouts'
    want_op = {'ps_base': None, 'ps_setm': 'setm', 'ps_reseed': 'reseed',
               'ps_reseed_setm': 'reseed_setm'}[ARMS[arm][0]]
    for r in rows:
...
def summarize(out, cells, arms, strict=False):
    ...
    return {'complete': complete, 'baseline_collection_mismatches': baseline_mismatch,
            'arms': totals}
...
def main():
    ...
    arms = a.arms.split(','); assert set(arms) <= set(ARMS) and len(set(arms)) == len(arms)
    ...
    for c, rows in sorted(cells.items()):
        ...
        det=a.main_root/f'outputs/analysis/grid_phase/detector_v6_ho/loko/{stem}/s{s}/j{j}/detector_pertask_lstm_{stem}.pt'
        assert det.is_file(), det
        verify(det)
        root=a.out/'npz_roots'/f'{slug}_s{s}_j{j}'
        (root/slug).mkdir(parents=True,exist_ok=True)
        for variant in ('instr_setm_v6_gt','instr_setm_v6_gt_plain'):
            target=npz/variant/art/f's{s}'/f'j{j}'
            assert list(target.glob('*/dit_L12/conceptors.npz')), target
        ...
        for arm in arms:
            ...
            if existing:
                try:
                    ...
                    check_sidecars(a.out,arm,c,existing)
    ...
    summary=summarize(a.out,cells,arms) if not a.dry_run else {'dry_run':True}
