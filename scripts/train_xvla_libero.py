...
class CustomTrainPipelineConfig(TrainPipelineConfig):
    # Add a new argument for filtering by task name for evaluation
    eval_target_task_name: Optional[str] = None
...
def wrap_policy_in_peft_model(cfg, policy):
    ...
    from lerobot.scripts.lerobot_train import get_default_peft_configuration
...
def train(cfg: CustomTrainPipelineConfig, accelerator: Accelerator | None = None):
    ...
    if is_main_process:
        ...
        # HACK: Force PyAV backend to avoid torchcodec issues
    ...
    if cfg.eval_freq > 0 and cfg.env is not None:
        if is_main_process:
            ...
            gym_kwargs = {}
            # We need to manually create Libero envs if we want to inject custom gym_kwargs easily
            # But make_env supports passing gym_kwargs via cfg.env.gym_kwargs if we modify cfg.
            # Let's modify cfg.env.gym_kwargs directly.
            if gym_kwargs:
                if cfg.env.gym_kwargs is None:
                    cfg.env.gym_kwargs = {}
                cfg.env.gym_kwargs.update(gym_kwargs)

            # Use create_libero_envs directly to ensure correct task filtering and structure
            env_cls = gym.vector.AsyncVectorEnv if cfg.eval.use_async_envs else gym.vector.SyncVectorEnv
            
            eval_env = create_libero_envs(
                task=cfg.env.task,
                n_envs=cfg.eval.batch_size,
                gym_kwargs=cfg.env.gym_kwargs,
    ...
    train_metrics = {
        ...
        "position_loss": AverageMeter("pos", ":.3f"),
        "rotate6D_loss": AverageMeter("rot", ":.3f"),
        "gripper_loss": AverageMeter("grip", ":.3f"),
        "joints_loss": AverageMeter("jnt", ":.3f"),
        "time_loss": AverageMeter("time", ":.3f"),
    ...
    # Get time_feature_key from policy config if available
    time_feature_key = getattr(cfg.policy, 'time_feature_key', None)
    
    for _ in range(step, cfg.steps):
        start_time = time.perf_counter()
        batch = next(dl_iter)
        
        # Preserve time_to_go before preprocessing (preprocessor drops non-standard keys)
        time_to_go = batch.get(time_feature_key) if time_feature_key else None
        
        batch = preprocessor(batch)
        
        # Re-insert time_to_go after preprocessing
        if time_to_go is not None:
            batch[time_feature_key] = time_to_go
        ...
        # Update custom metrics from output_dict
        # Action losses (depends on action_mode: ee6d, joints, auto, etc.)
        if "position_loss" in output_dict:
            train_tracker.position_loss = output_dict["position_loss"]
        if "rotate6D_loss" in output_dict:
            train_tracker.rotate6D_loss = output_dict["rotate6D_loss"]
        if "gripper_loss" in output_dict:
            train_tracker.gripper_loss = output_dict["gripper_loss"]
        if "joints_loss" in output_dict:
            train_tracker.joints_loss = output_dict["joints_loss"]
        # Time loss
        if "time_loss" in output_dict:
            train_tracker.time_loss = output_dict["time_loss"]
        ...
        if cfg.env and is_eval_step:
            if is_main_process:
                ...
                with torch.no_grad(), accelerator.autocast():
                    ...
                    eval_info = eval_policy_all_custom(
