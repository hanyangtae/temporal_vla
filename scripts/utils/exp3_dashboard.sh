#!/usr/bin/env bash
# exp3(구 pq3) read-only 전황 대시보드 — 어디서 무슨 작업이 돌고 있는지 한 화면 요약.
# 사용:  watch -n 30 bash scripts/utils/exp3_dashboard.sh        (30초 갱신)
#        bash scripts/utils/exp3_dashboard.sh                    (1회 출력)
#
...
SE="$REPO/outputs/eval/robocasa/groot_n15/steer_eval_exp3"
RR="$REPO/outputs/eval/robocasa/groot_n15/phase_event_exp3/raw_rollouts"
RQ='pkt_ws/temporal_vla/outputs/eval/robocasa/groot_n15/steer_eval_pq3'  # 워커(srv50/srv48, 구 w2·w50/w48) 원격 — 원격은 구명(pq) 유지
SSH_OPT=(-o ConnectTimeout=5 -o BatchMode=yes)
TMP=$(mktemp -d); trap 'rm -rf "$TMP"' EXIT

# ── 원격 수집 (병렬) ──────────────────────────────────────────────────────────
ssh -p 11112 "${SSH_OPT[@]}" kimseungjun@166.104.146.37 '
  p=$(pgrep -af "fit_phase_conceptor|exp3_gated_gate" | grep -v pgrep | sed "s/^/  proc: /" | cut -c1-110)
...
ssh "${SSH_OPT[@]}" junhyeong@166.104.35.50 '
  ...
  for f in /tmp/pq3_lane_srv50_*.log; do [ -f "$f" ] && echo "  $(basename $f .log): $(tail -1 $f | cut -c1-95)"; done
...
' > "$TMP/srv50" 2>/dev/null || echo "  unreachable" > "$TMP/srv50" &
...
ssh "${SSH_OPT[@]}" junhyeong@166.104.35.48 '
  ...
  q=~/'"$RQ"'/work_queue
  echo "  E큐: 남음 $(grep -c . $q/queue.tsv 2>/dev/null || echo 0) · 실행중 $(ls $q/running 2>/dev/null | wc -l) · 완료 $(wc -l < $q/done/done.tsv 2>/dev/null || echo 0) · 실패 $(wc -l < $q/failed/failed.tsv 2>/dev/null || echo 0)"
  for f in /tmp/pq3_lane_srv48_*.log; do [ -f "$f" ] && echo "  $(basename $f .log): $(tail -1 $f | cut -c1-95)"; done
  ...
  pgrep -f select_instruction_seeds >/dev/null && echo "  스캔 프로세스: ALIVE" || echo "  스캔 프로세스: 없음"
' > "$TMP/srv48" 2>/dev/null || echo "  unreachable" > "$TMP/srv48" &

# ── 로컬 수집 ────────────────────────────────────────────────────────────────
{
  nvidia-smi --query-gpu=index,memory.used,utilization.gpu --format=csv,noheader 2>/dev/null \
    | awk -F', ' '$1<=2 {print "  GPU"$1": "$2" / util "$3}'
  p=$(pgrep -af 'exp3_collect_cell|beta_sweep|exp3_cell_runner|exp3_c0_scan|ppcc_sweep_chain' \
...
echo "exp3 전황판  $(date '+%F %T')  (read-only · watch -n 30 권장)"
hr
...
hr
echo "[a100 50]   E lane(GPU0) · pizza 스캔"; cat "$TMP/srv50"
hr
echo "[a100 48]   E lane(GPU2) · beer/pizza 스캔"; cat "$TMP/srv48"
hr
