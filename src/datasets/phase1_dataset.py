...
class Phase1WindowDataset(Dataset):
    """
    Phase 1 ProgressPredictor 학습용 데이터셋 (VITA-faithful window 방식).
    ...
    def __init__(
        ...
        print(f"Phase1WindowDataset: {self._n_episodes} episodes → {len(self._windows)} windows "
              f"(w_tr={window_size}, k={max_windows_per_episode})")
    ...
    def _build_windows(self) -> list:
        """
        에피소드별로 dissimilarity sampling → window 리스트 구성.

        Returns:
            list of (ep_from, window_start, ep_length) tuples
        """
        windows = []
    ...
    def _select_diverse_windows(self, ep_from: int, ep_length: int) -> list:
        """
        VITA Eq.5 dissimilarity sampling → 선택된 window start indices.

        Returns:
            list of window start indices (에피소드 내 상대 위치)
        """
        stride = 1
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Window 임베딩 시퀀스 [w_tr, D]
        z_seq = torch.stack([
            self._embeddings[ep_from + window_start + t]
            for t in range(self.window_size)
        ])

        # 절대 진행률 label: (window_start + t) / (T - 1)
        targets = torch.tensor([
            (window_start + t) / max(ep_length - 1, 1)
            for t in range(self.window_size)
        ], dtype=torch.float32).unsqueeze(-1)  # [w_tr, 1]

        return {
            "z_seq": z_seq,      # [w_tr, D]
            "targets": targets,  # [w_tr, 1]
        }
...
class Phase1EpisodicDataset(Dataset):
    ...
    def _compute_pred_mask(self, ep_from: int, ep_length: int) -> torch.Tensor:
        ...
        if len(candidates) <= self.max_windows_per_episode:
            ...
            # 각 candidate window의 frame embedding을 concat (frame-wise distance 비교)
            feats = torch.stack([
                torch.cat([
                    self._embeddings[ep_from + i]
                    for i in range(s, s + self.window_size)
            ...
            # VITA Eq.5: s(w) = Σ_{v∈W} ||w - v||² (concat → frame-wise L2 distance 합)
