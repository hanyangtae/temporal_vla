...
class LLaVAInstructDataset(Dataset):
    ...
    def __init__(self, tokenizer, data_file_path=None, processor=None):
        ...
        self.image_root = data_file_path

        with open(data_file_path + '/llava_v1_5_mix665k.json', 'r') as f:
            data = json.load(f)
        self.list_data_dict = []
        for item in data:
    ...
    def __getitem__(self, i):
        ...
        try:
            image = Image.open(os.path.join(image_folder, image_file)).convert('RGB')  # hwc, 255
...
def get_instruct_data_loader(
    ...
    train_dataset = LLaVAInstructDataset(tokenizer, processor=processor, data_file_path=dataset_path)
    ...
    if return_system_prompt:
        return dataloader, SYSTEM_PROMPT, SYSTEM_PROMPT_LEN
    return dataloader
