...
class UniversalPrompting_w_action():
    ...
    def __init__(self,
        ...
        self.sptids_dict = {
            token: torch.tensor(self.text_tokenizer.convert_tokens_to_ids([token])) for token in special_tokens
        }
    ...
    def pre_prompt(self, text_ids, image_ids, labels):
        ...
        for i in range(len(text_ids)):
            ...
            if probs[i] < self.cond_dropout_prob:
                temp_ids = [
                    int(self.sptids_dict['<|t2i|>']), self.text_tokenizer.bos_token_id, self.text_tokenizer.eos_token_id
                ]
...
def create_attention_mask_for_mmu_vit(
    ...
    if return_inverse_mask:
        ...
        inverted_mask = inverted_mask.masked_fill(inverted_mask.to(torch.bool), torch.iinfo(torch.int64).min)
        return inverted_mask
    else:
        return causal_mask


def create_attention_mask_predict_next_for_future_prediction(sequence,
                                                             pad_id=128256,
                                                             soi_id=128257,
                                                             eoi_id=128258,
    ...
    in_image_segment = (cumulative_start > cumulative_end) | is_start_image | is_end_image  # 1+1024+1

    is_text = ~(in_image_segment)  # b,l

    causal_mask = torch.tril(torch.ones((L, L), dtype=torch.bool)).to(sequence.device)

    mask_text = is_text[:, :, None] * causal_mask[None, :, :]  # B,L,L (b,1603+10,1603+10)
    is_text_image = is_text | in_image_segment
    mask_text_image_bi = is_text_image[:, :, None] * is_text_image[:, None, :]  # B,L,L (b,1603+10,1603+10)
    if rm_pad_in_image:
