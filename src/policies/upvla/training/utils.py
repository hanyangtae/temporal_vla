...
def mask_or_random_replace_tokens(image_tokens, mask_id, config, mask_schedule, is_train=True):
    ...
    if (config.training.get("predict_all_tokens", False) or
            config.training.get("noise_type", "mask") == "random_replace"):
        labels = image_tokens
...
def mask_or_random_replace_tokens_for_future_prediction(input_image_tokens,
                                                        target_image_tokens,
                                                        mask_id,
                                                        config,
                                                        mask_schedule,
                                                        is_train=True):
    input_ids = input_image_tokens  # no mask, directly reconstruct new prediction
    labels = target_image_tokens
    loss_weight = None
    mask_prob = None
    return input_ids, labels, loss_weight, mask_prob
