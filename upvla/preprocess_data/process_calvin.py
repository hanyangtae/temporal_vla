...
def preprocess_dataset(origin_dataset_path):
    ...
    lang_info = np.load(
        os.path.join(origin_dataset_path, "lang_annotations", "auto_lang_ann.npy"),
        allow_pickle=True,
    ).item()
    ...
    dataset_info = []
    for i, ((episode_start, episode_end), instruction) in tqdm.tqdm(
        enumerate(zip(episode_start_end, texts))
    ):
        # if i < 10:
        ...
        for j, frame_idx in enumerate(range(episode_start, episode_end + 1)):
            file_path = os.path.join(
                origin_dataset_path, "episode_%07d.npz" % frame_idx
            )
...
preprocess_dataset(
    "/mnt/calvin/task_ABC_D/validation"
)  # change the path to your own raw calvin abcd dataset
